---
name: Ink TUI
description: Ink 6 + React terminal UI layout, borders, wrapping, and overflow. USE WHEN building or fixing emergex-code TUI, chat bubbles, footers, Ink Box/Text layout, text on border lines, or terminal overflow.
---

# Ink + React TUI specialist (emergex-code)
...
## emergex-code project rules (non-negotiable)
...
If text still overflows horizontally, **`truncate()`** or **`wrapText`** in `apps/tui/src/lib/text` is preferred for emergex-code consistency.
