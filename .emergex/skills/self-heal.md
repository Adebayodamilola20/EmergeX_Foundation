...
When emergex encounters errors, it doesn't stop and ask. It diagnoses, adapts, and continues. User sees progress in the feed but isn't interrupted.
...
For each operation, emergex has fallback strategies:
...
```
[emergex:heal] Error: npm install timed out
[emergex:heal] Retrying with bun install...
[emergex:heal] Success. Continuing.

[emergex:heal] Error: AST parser failed on malformed file
[emergex:heal] Falling back to grep...
[emergex:heal] Found 3 matches. Continuing.

[emergex:heal] Error: Model returned invalid JSON (attempt 1/3)
[emergex:heal] Rephrasing prompt...
[emergex:heal] Error: Model returned invalid JSON (attempt 2/3)
[emergex:heal] Switching to different model...
[emergex:heal] Success. Continuing.
```

## Error Memory

emergex remembers what worked:

```json
// .emergex/healing-memory.json
{
  "patterns": {
    "npm-timeout": {
      "count": 5,
...
emergex stops self-healing and asks user when:
...
**Remember:** A good agent doesn't break on first error. It adapts. It tries alternatives. It learns. The user hired emergex to solve problems, not to report every obstacle.
