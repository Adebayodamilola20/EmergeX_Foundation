...
emergex can modify its own code, tools, and configuration when stuck or when improvements are needed. Every modification is versioned, tested, and rollback-ready.
...
## What emergex Can Modify

### Level 1: Configuration (Low Risk)
- `.emergex/config.json` - Settings, defaults
- `.emergex/skills/*.md` - Skill definitions
...
- `packages/emergex/index.ts` - Agent logic
...
2. BRANCH
   git checkout -b emergex/self-<description>-<timestamp>

3. DOCUMENT
   Log: "[emergex:self] Modifying <file> because <reason>"
```
...
2. If ALL pass:
   - Merge to main
   - Log: "[emergex:self] Evolution successful: <summary>"
3. If ANY fail:
   - Stay on branch
   - Log: "[emergex:self] Evolution rejected: <reason>"
   - Return to main, stash preserved
```

## Self-Modification Triggers

### Automatic (emergex decides)
...
Every self-modification is logged to `.emergex/evolution.log`:
...
[2024-03-11T09:45:00Z] MODIFY packages/toolshed/tools/search.ts
  ...
  BRANCH: emergex/self-search-timeout-1710234567
  MERGED: true

[2024-03-11T09:50:00Z] MODIFY packages/emergex/index.ts
  REASON: Need to handle streaming responses
  CHANGE: Added StreamingClient class
  RESULT: FAILED - type errors in 3 files
  BRANCH: emergex/self-streaming-1710234567
  MERGED: false
...
cat .emergex/evolution.log | grep "RESULT: FAILED"
...
```
[emergex:self] Detected: timeout error (3rd occurrence)
[emergex:self] Analyzing: packages/toolshed/tools/search.ts
[emergex:self] Branch: emergex/self-search-fix-1710234567
[emergex:self] Change: Increased timeout 30s → 120s
[emergex:self] Testing...
[emergex:self] Tests: 14/14 passing
[emergex:self] Merged to main
[emergex:self] Evolution complete. Search now handles large codebases.
```
