...
emergex remembers what it was doing, what worked, what failed, and what the user prefers. Every session builds on the last.
...
```json
// .emergex/context/working.json
{
  "session": "sess_1710234567",
  "started": "2024-03-11T09:45:00Z",
  "project": "emergex-code",
  "branch": "emergex/feat-auth-1710234567",
...
```json
// .emergex/context/project.json
{
  "projectName": "emergex-code",
...
When emergex starts a new session:
...
5. Log: "[emergex] Resuming: <last task>"
```

Feed output:
```
[emergex] Session restored
[emergex] Last task: Implementing OAuth2 login
[emergex] Branch: emergex/feat-auth-1710234567
[emergex] Files: src/auth.ts, src/login.tsx
[emergex] Ready to continue.
```

## Session End

When emergex ends a session:
...
emergex can query its own memory:
...
```
[emergex:memory] Saved: working context
[emergex:memory] Learned: bun faster than npm for this project
[emergex:memory] Remembered: previous auth implementation pattern
```
...
- Stored in `.emergex/context/`
- Never uploaded anywhere
- Can be wiped with `rm -rf .emergex/context/`
...
These go in `~/.emergex/global/` (user home).
