...
All notable changes to emergex Code will be documented in this file.
...
- **GitHub Projects playbook** - `docs/process/GITHUB-PROJECTS.md` and `scripts/gh-project-roadmap.sh` for org Project setup, linking `emergex-code`, and bulk-adding epic **#1076** and **#1077–#1093**. Org project [**emergex-code roadmap**](https://github.com/orgs/8gi-foundation/projects/1) (number `1`) is live and linked to the repo.
...
### Changed
- **Repository scope** - Ecosystem governance markdown (`docs/8GI-*.md`, `8GO-SPEC.md`, presentation scripts) and product PRD drafts moved to [`8gi-governance`](https://github.com/8gi-foundation/8gi-governance) and [`emergex-world`](https://github.com/8gi-foundation/emergex-world). Static `media/` decks and avatars now live under `emergex-world` (`public/media/`). This repo keeps kernel and agent documentation only.
- **Housekeeping** - README, SECURITY, CODEOWNERS, `quarantine/safe-access.md`, `scripts/research-log.json`, `scripts/utility-queue.json`, `bun.lock`, `.emergex/user.json`. `.gitignore` now ignores local `claude-code/` reference trees (nested `.git`).
...
- **Harbor adapter** - Fixed AgentContext API (pydantic data model, not message store). Added robust Bun + emergex installation in Docker containers.

### Added
- **Terminal-Bench Harbor adapter** (`benchmarks/harbor_adapter/`) - Runs emergex through Terminal-Bench 2.0 via Harbor framework. Oracle baseline validated at 80%.
...
- **Running Apps menu** in Lil EmergeX's macOS menu bar - memory stats, per-app quit/force-quit dialogs, Quit All Non-Essential, protected app list.
...
### Added
- **Daemon Protocol** (`docs/DAEMON-PROTOCOL.md`) - WebSocket protocol specification for external clients (emergex.app, emergex OS, Telegram). Defines connection handshake, auth, session lifecycle, prompt/response streaming, cron management, and health checks. The contract between the brain and the interfaces.
- **BRAND.md** - Canonical brand reference copied from emergex-world. Typography (Fraunces/Inter/JetBrains Mono), color palette, domain table.
...
- **Session state persistence** (`packages/daemon/index.ts`) - Saves active session metadata to `~/.emergex/daemon-state.json` on graceful shutdown. Clients can resume sessions after daemon restart.
...
- **Ecosystem references** - CLAUDE.md and README.md now reference emergex.dev as canonical domain and link to all ecosystem products (emergexos.com, emergex.app, emergexjr.com, emergex.world, emergex.games).

### Changed
- **CLAUDE.md** - Added ecosystem table and reference to BRAND.md. emergex Code positioned as "the brain" and free on-ramp to emergex OS.
...
- **Curriculum Skills** (`packages/emergex/curriculum.ts`) - Teachable curricula with step progression, exercises, and comprehension checks. Built-in: "emergex-architecture" (5 steps) and "writing-benchmarks" (4 steps). `CurriculumRunner` generates teaching prompts.
...
- **Healing** (`packages/validation/`) - Hypothesis Loop pattern: checkpoint-action-verify-revert. Git-stash atomic snapshots, failure log (`~/.emergex/healing/failures.jsonl`), configurable retry limits
...
### Added
- **`apps/clui/` — Tauri 2.0 desktop overlay** — branded emergex desktop app with Alt+Space toggle, multi-tab sessions, transparent floating overlay, Rust backend for process management, React 19 frontend with Tailwind CSS 4, real-time NDJSON streaming from agent subprocess, permission server for human-in-the-loop tool approval
- **`packages/auth/` — Clerk authentication** — device code flow for CLI login (`emergex auth login`), macOS Keychain token storage with AES-256-GCM encrypted file fallback, JWT validation via `jose`, automatic token refresh, non-blocking anonymous mode (auth never blocks local usage)
...
- **`packages/control-plane/` — Multi-tenant management** — tenant provisioning with subdomain routing (username.emergex.app), usage analytics, billing plan definitions (free/pro/team), Stripe integration stubs, admin dashboard data layer
...
- **CLI auth commands** — `emergex auth login`, `emergex auth logout`, `emergex auth status`, `emergex auth whoami`
...
- **Vision config in `.emergex/config.json`** — user-configurable `defaultModel`, `ocrModel`, fallback chains, provider preference (local/cloud), timeout
...
- **write_file path bug** — models sometimes pass absolute-looking paths like `/emergex-code/server.ts` that resolve outside the working directory; these are now auto-stripped to relative paths instead of throwing a path-traversal error; tool description and system prompt updated to instruct models to use relative paths
...
- **emergex as default provider** — `eight-1.0-q3:14b` is now the primary recommended model across all documentation and quick-start guides
...
### Fixed
- **Security fixes ported to `packages/emergex`** — hardened command execution, input sanitization, and permission checks carried over from agent package

### Added (prior)
- **`@emergex/kernel` package** — full 4-phase RL fine-tuning pipeline via training proxy
  ...
  - **Kernel manager** (`manager.ts`) — unified entry point, reads `.emergex/config.json`, safe no-op when disabled
...
- **Training proxy toggle** — `TRAINING_PROXY_URL` env var and `.emergex/config.json` training_proxy section to route Ollama calls through the OpenAI-compatible training proxy
...
- **Remotion video demos** (`apps/demos/`) — React-based video generation for product reels and landing page content
  ...
  - Branded design tokens matching emergex visual identity
...
### Added
- **packages/emergex/** — New core agent engine (replaces packages/agent/)
...
- **packages/harness-cli/** — Headless CLI for running and inspecting emergex sessions
...
### Changed
- **Breaking:** `packages/agent/` renamed to `packages/emergex/`
...
### Fixed
- .env loading from repo root and ~/.emergex when running from another directory
