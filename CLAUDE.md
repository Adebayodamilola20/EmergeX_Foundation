...
emergex Code - the kernel of the emergex ecosystem. Open source autonomous coding agent TUI powered by local LLMs (Ollama) or free cloud models (OpenRouter). The free on-ramp to emergex OS.
...
| **emergex** | emergex.app | The product - single pane of glass dashboard for all emergex services. |
| **emergex World** | emergex.world | Ecosystem story, docs, media. |
| **emergex Games** | emergex.games | Agent simulation playground. |
| **emergex Jr** | emergexjr.com | AI assistant for kids. Accessibility first. Free. |
| **emergex Telegram** | t.me/eaborobot | Telegram Mini App - mobile-first emergex interface. |
...
| Repo | Role |
|------|------|
| `emergex-code` | Open source agent kernel (this repo) |
| `emergex-world` | Docs, story, marketing site (emergex.world) |
| `emergex-app` | Single pane of glass dashboard (emergex.app) |
| `emergex-telegram-app` | Telegram Mini App + bot |
| `control-plane` | Board Plane - multi-vessel orchestration |
| `board-vessel` | Board member vessel runtime |
| `lil-emergex` | Swift dock pet companion |
| `emergex-games` | AI civilisation simulator |
| `emergexjr` | AI OS for neurodivergent children |

The [emergex Constitution](https://emergex.world/constitution) governs all decisions.
...
```bash
# Users
npm install -g @8gi-foundation/emergex-code       # install globally
emergex                                     # launch anywhere
...
| File | What it does |
|------|--------------|
| `packages/emergex/tools.ts` | Core tool definitions for the agent |
| `packages/emergex/agent.ts` | Agent loop, abort, checkpoint restore |
| `packages/emergex/prompts/system-prompt.ts` | System prompt with user context injection |
...
bun -e "import {memoryHealth} from './packages/memory/health.ts'; import {Database} from 'bun:sqlite'; console.log(memoryHealth(new Database('.emergex/memory.db')))"
...
The `@emergex/kernel` package handles continuous RL fine-tuning via a training proxy. Key files:
...
**Data dir:** `.emergex/kernel/`

The pipeline is **off by default** - set `"training_proxy": { "enabled": true }` in `.emergex/config.json` to activate.
