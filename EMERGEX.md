# EMERGEX.md — The emergex Development Process

This file defines how emergex is built. It's different from CLAUDE.md (which is for any Claude agent working on this repo). This is specifically about emergex's philosophy, architecture decisions, and the autoresearch-driven development loop.

---

## Philosophy

emergex is a **prodigy, not a product.** It's a system that learns from its own failures.
...
```
packages/emergex/       ← The brain. Agent engine, REPL, tools, prompt system.
packages/ai/          ← AI SDK integration. Provider abstraction. Tool loop.
packages/harness-cli/ ← Headless CLI for testing emergex sessions.
...
**packages/emergex/ is sacred.** It is the core engine. Treat it with care. Read before you write. Understand before you modify.
...
source ~/emergex-code/.env && export OPENROUTER_API_KEY
...
| Thomas Davis (@thomasdavis) | TUI, AI SDK, packages/emergex refactor |
