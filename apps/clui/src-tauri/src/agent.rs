/**
 * emergex CLUI — Agent Session Manager
 *
 * Manages Bun child processes, one per session tab.
 * Each process runs `bun run packages/emergex/index.ts` and communicates
...
/// NDJSON event emitted by the emergex engine.
...
impl AgentManager {
    ...
    fn resolve_repo_root() -> std::path::PathBuf {
        ...
        if let Ok(root) = std::env::var("EIGHT_REPO_ROOT") {
            ...
            if p.join("packages/emergex/clui-bridge.ts").exists() {
                return p;
            }
        }
        ...
        if let Some(root) = from_manifest {
            if root.join("packages/emergex/clui-bridge.ts").exists() {
                return root.to_path_buf();
            }
        }

        // 3. Walk up from current_dir looking for packages/emergex/clui-bridge.ts
        if let Ok(cwd) = std::env::current_dir() {
            let mut dir = cwd.as_path();
            for _ in 0..10 {
                if dir.join("packages/emergex/clui-bridge.ts").exists() {
    ...
    pub fn create_session(
        ...
        // Resolve the emergex repo root reliably
