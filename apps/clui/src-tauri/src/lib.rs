...
fn setup_tray(app: &AppHandle) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // -- Info --
    let version = MenuItem::with_id(app, "version", "emergex Code v1.0.0", false, None::<&str>)?;
    let quit = MenuItem::with_id(app, "quit", "Quit emergex CLUI", true, None::<&str>)?;
    ...
    let _tray = TrayIconBuilder::new()
        ...
        .tooltip(if daemon_up {
            format!("emergex - {} session{}", sessions, if sessions == 1 { "" } else { "s" })
        } else {
            "emergex - Daemon offline".to_string()
        }.as_str())
        .on_menu_event(move |app, event| {
            match event.id.as_ref() {
                ...
                "open_terminal" => {
                    // Open a new terminal window running emergex TUI
                    ...
                    let _ = Command::new("osascript")
                        .args([
                            "-e",
                            "tell application \"Terminal\" to do script \"emergex\"",
                        ])
                        .spawn();
                }
                "suggest_quit" => {
...
pub fn run() {
    tauri::Builder::default()
        ...
        .expect("error while running emergex CLUI");
}
