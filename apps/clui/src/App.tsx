/**
 * emergex CLUI — Main Application Shell
...
export function App() {
  ...
  const [themeMode, setThemeMode] = useState<ThemeMode>(() => {
    try { return (localStorage.getItem("emergex-theme") as ThemeMode) || "dark"; } catch { return "dark"; }
  });
  ...
  // Persist theme
  useEffect(() => {
    try { localStorage.setItem("emergex-theme", themeMode); } catch {}
    document.body.style.backgroundColor = t.bg;
  }, [themeMode, t.bg]);

  // Inject keyframes for pulse animation
  useEffect(() => {
    const styleId = "emergex-keyframes";
  ...
  function handleNewTab() {
    ...
    if (IS_TAURI) {
      setTimeout(() => {
        addMsgToTab(newTab.id, "system", "emergex CLUI — New Session");
      ...
      <form onSubmit={handleSubmit} style={{
        ...
        <div style={{
          ...
          <input
            ...
            placeholder={activeTab?.isProcessing ? "Agent is working..." : "Ask emergex anything... (/help for commands)"}
