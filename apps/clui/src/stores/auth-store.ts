/**
 * emergex CLUI -- Auth Store (Zustand)
 *
 * Wraps the AuthManager from @emergex/auth into a reactive Zustand store.
...
// ── Types (mirrored from @emergex/auth to avoid build-time dep issues) ──
...
async function getAuthManager(): Promise<any> {
  if (!_authManagerPromise) {
    _authManagerPromise = import("@emergex/auth").then((mod) =>
      mod.getAuthManager(),
    ).catch(() => null);
  }
