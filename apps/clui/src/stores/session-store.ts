/**
 * emergex CLUI -- Session Store (Zustand)
 *
 * Manages multi-tab session state. Each session represents an independent
 * emergex agent subprocess with its own message history, status, and metadata.
 *
  ...
  sendMessage: (sessionId, content) => {
    ...
    setTimeout(() => {
      const assistantMsg: Message = {
        id: generateMessageId(),
        role: "assistant",
        content: `Received: "${content}"\n\nThis is a placeholder response. In production, this would come from the emergex engine subprocess via NDJSON event streaming.`,
        timestamp: new Date(),
      };
