...
import { PLAN_DEFINITIONS } from "@emergex/control-plane";
              ...
              {isLoading ? (
                <div className="space-y-3 animate-pulse">
                  {Array.from({ length: 4 }).map((_, i) => (
                    <div key={i} className="h-6 rounded bg-[var(--emergex-bg-hover)]" />
                  ))}
                </div>
              ) : (
              ...
              {isLoading ? (
                ...
                <div className="space-y-2">
                      ...
                      return (
                        <div key={model}>
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-[var(--emergex-text-secondary)] truncate max-w-[140px]">
                              {model}
                            </span>
                            <span className="text-[var(--emergex-text-muted)]">
                              {count} ({pct.toFixed(0)}%)
                            </span>
                          </div>
                          <div className="mt-1 h-1.5 w-full rounded-full bg-[var(--emergex-bg-hover)]">
                            <div
                              className="h-1.5 rounded-full bg-[var(--emergex-accent)]"
                              style={{ width: `${pct}%` }}
                            />
                          </div>
                        </div>
                  ...
                  {Object.keys(health.modelDistribution).length === 0 && (
                    <p className="text-xs text-[var(--emergex-text-muted)]">No data yet</p>
                  )}
                </div>
              )}
            </div>
                ...
                <div className="space-y-2">
                  {Object.entries(dashboard.planDistribution).map(([plan, count]) => (
                    <div key={plan} className="flex items-center justify-between text-sm">
                      <span className="text-[var(--emergex-text-secondary)] capitalize">
                        {plan}
                      </span>
                      <span className="text-[var(--emergex-text)]">{count}</span>
                    </div>
                  ))}
                </div>
              )}
...
function HealthRow({
  ...
  const statusColors = {
    success: "text-[var(--emergex-success)]",
    warning: "text-[var(--emergex-warning)]",
    error: "text-[var(--emergex-error)]",
  };

  return (
    <div className="flex items-center justify-between">
      <span className="text-xs text-[var(--emergex-text-muted)]">{label}</span>
      <span
        className={`text-sm font-medium ${status ? statusColors[status] : "text-[var(--emergex-text)]"}`}
      >
        {value}
      </span>
    </div>
