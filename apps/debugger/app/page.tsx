...
export default function Home() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("emergex-debugger-theme");
    if (saved === "light") {
      setIsDark(false);
  ...
  const toggleTheme = () => {
    ...
    if (next) {
      ...
      localStorage.setItem("emergex-debugger-theme", "dark");
    } else {
      document.documentElement.classList.remove("dark");
      localStorage.setItem("emergex-debugger-theme", "light");
    }
  };
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center" style={{ color: "var(--muted)" }}>
              ...
              <p className="text-xs mt-2" style={{ color: "var(--border)" }}>
                Sessions stream live from ~/.emergex/sessions
              </p>
            </div>
          </div>
        )}
