        ...
        Task {
            do {
                ...
                guard let display = content.displays.first else {
                    print("[lil-emergex] No display found")
                ...
                print("[lil-emergex] Screenshot error: \(error)")
                DispatchQueue.main.async { completion(nil) }
            }
        }
    }
...
class SpriteManager {
    ...
    func loadFromAtlas(path: String, manifestPath: String) {
        guard let atlasImage = NSImage(contentsOfFile: path),
            ...
            print("[lil-emergex] Failed to load sprite atlas from \(path)")
            return
        }
        ...
        print("[lil-emergex] Loaded \(animations.count) animations")
    }
}
...
class SoundManager {
    ...
    func loadSounds(fromDir dir: String) {
        ...
        guard let files = try? fm.contentsOfDirectory(atPath: dir) else {
            print("[lil-emergex] No sounds directory at \(dir)")
            return
        }
        ...
        print("[lil-emergex] Loaded \(players.count) sounds")
    }
...
class ChatPopoverView: NSView, NSTextFieldDelegate {
    ...
    func setupUI() {
        ...
        // Input area
        inputField.placeholderString = "Talk to EmergeX..."
        ...
        // Welcome message
        appendMessage(ChatMessage(role: .system, text: "Click send or press Enter to chat with EmergeX"))
    }
...
struct CompanionData {
    ...
    static func load() -> CompanionData? {
        ...
        let filePath = "\(home)/.emergex/active-companion.json"
...
class CompanionCardView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        let home = FileManager.default.homeDirectoryForCurrentUser.path
        let bodyPath = "\(home)/emergex-code/apps/lil-emergex/parts/\(companion.species)/body.png"
        ...
        let footStr = NSAttributedString(string: "emergex.dev", attributes: footAttrs)
...
class PetController {
    ...
    init(sessionId: String, spriteManager: SpriteManager, soundManager: SoundManager, startX: CGFloat) {
        ...
        // Load companion identity from ~/.emergex/active-companion.json
        if sessionId == "eight" {
            self.companionData = CompanionData.load()
            if let comp = self.companionData {
                print("[lil-emergex] Companion loaded: \(comp.fullName) (\(comp.species), \(comp.element))")
    ...
    func ensureChatSession() {
        ...
        if sessionId != "eight" {
            ...
            manager.daemonClient.send(["type": "session:create", "channel": "lil-emergex"])
        }
    }
    ...
    func sendChatMessage(_ text: String) {
        ...
        // /do prefix - execute tasks via shell (the EmergeX way)
    ...
    func handleRightClick(_ event: NSEvent) {
        ...
        // Header
        let headerTitle = sessionId == "eight" ? (companionData?.fullName ?? "Lil EmergeX") : "Agent: \(Self.shortId(sessionId))"
...
class DaemonClient {
    ...
    func connect() {
        ...
        webSocket?.resume()
        print("[lil-emergex] Connecting to ws://localhost:\(port)...")
    ...
    func send(_ dict: [String: Any]) {
        ...
        webSocket?.send(.string(string)) { error in
            if let error = error {
                print("[lil-emergex] Send error: \(error.localizedDescription)")
            }
        }
    }
    ...
    func receiveLoop() {
        webSocket?.receive { [weak self] result in
            ...
            case .failure(let error):
                print("[lil-emergex] WebSocket error: \(error.localizedDescription)")
                DispatchQueue.main.async {
    ...
    func handleMessage(_ msg: [String: Any]) {
        ...
        switch type {
        case "auth:ok":
            print("[lil-emergex] Authenticated")
            send(["type": "sessions:list"])

        case "auth:fail":
            print("[lil-emergex] Auth failed!")
        ...
        case "error":
            ...
            print("[lil-emergex] Daemon error: \(message)")
    ...
    func scheduleReconnect() {
        ...
        reconnectTimer = Timer.scheduledTimer(withTimeInterval: 5.0, repeats: false) { [weak self] _ in
            print("[lil-emergex] Reconnecting...")
            self?.connect()
        }
    }
...
class PetManager: DaemonClientDelegate {
    ...
    func startDaemon() {
        // Find emergex-code repo: check env, then common locations
        let eightCodePath: String = ProcessInfo.processInfo.environment["EIGHT_CODE_PATH"]
            ?? {
                let candidates = [
                    ...
                    NSString(string: "~/emergex-code").expandingTildeInPath,
                    NSString(string: "~/Code/emergex-code").expandingTildeInPath,
                    NSString(string: "~/.emergex/repo").expandingTildeInPath,
                ]
                ...
                return NSString(string: "~/emergex-code").expandingTildeInPath
            }()
    ...
    static func readDaemonConfig() -> (Int, String?) {
        let configPath = NSString(string: "~/.emergex/config.json").expandingTildeInPath
    ...
    func spawnPet(sessionId: String) {
        ...
        pet.petManager = self
        pets[sessionId] = pet
        print("[lil-emergex] Spawned pet for session: \(sessionId)")
        soundManager.play("hum")
    ...
    func despawnPet(sessionId: String) {
        ...
        print("[lil-emergex] Despawned pet for session: \(sessionId)")
    ...
    func daemonDidConnect() {
        daemonOnline = true
        print("[lil-emergex] Connected to EmergeX daemon")
    ...
    func daemonDidDisconnect() {
        daemonOnline = false
        print("[lil-emergex] Disconnected from EmergeX daemon")
        appDelegate?.updateMenu()
    }
    ...
    func daemonDidReceiveEvent(event: String, sessionId: String, payload: [String: Any]) {
        ...
        case "approval:required":
            ...
            sendNotification(title: "Lil EmergeX - Approval Needed", body: "Agent needs permission for: \(tool)")
    ...
    func daemonDidReceiveHealth(_ data: [String: Any]) {
        ...
        print("[lil-emergex] Health: \(sessions) sessions, up \(hours)h\(mins)m")
    }

    // MARK: - Computer Use (Daemon-driven)

    func daemonRequestsScreenshot(requestId: String) {
        print("[lil-emergex] Daemon requesting screenshot (id: \(requestId))")
        computerEngine.captureScreen { [weak self] base64 in
            guard let base64 = base64 else {
                print("[lil-emergex] Screenshot capture failed")
                return
            }
    ...
    func daemonRequestsAction(_ action: [String: Any]) {
        ...
        print("[lil-emergex] Executing: \(actionStr) - \(computerAction.reasoning ?? "")")
    ...
    // MARK: - CLI Task Execution (the EmergeX way)
    ...
    func executeTask(_ task: String, pet: PetController) {
        ...
        let systemPrompt = """
        You are EmergeX, an AI agent on macOS. Execute tasks using shell commands.
    ...
    func chatLocally(_ text: String, pet: PetController) {
        ...
        case .planning:
            systemPrompt = "You are EmergeX, a planning assistant. Help organize tasks and next steps. Be concise and actionable."
        case .studyPal:
            systemPrompt = "You are EmergeX, a study partner. Quiz the user, explain concepts. Be engaging and concise."
        case .coding:
            systemPrompt = "You are EmergeX, a helpful AI assistant on macOS. Be concise and direct. Keep responses under 2 sentences."
        }
...
// MARK: - Agent Mesh Monitor (reads ~/.emergex/mesh/)
...
class MeshMonitor {
    ...
    init() {
        ...
        self.meshDir = "\(home)/.emergex/mesh"
    }
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if let comp = CompanionData.load(), let speciesAtlas = comp.spriteAtlas {
            ...
            if FileManager.default.fileExists(atPath: resourceAtlasPath) {
                ...
                print("[lil-emergex] Loaded species atlas: \(speciesAtlas)")
                ...
                print("[lil-emergex] Loaded species atlas (dev): \(speciesAtlas)")
            }
        }
        ...
        if spriteManager.animations.isEmpty {
            print("[lil-emergex] No sprites! Run: bun run apps/lil-emergex/generate-sprites.ts")
        }
    ...
    func updateMenu() {
        ...
        let header = NSMenuItem(title: "Lil EmergeX v0.2.0", action: nil, keyEquivalent: "")
        header.isEnabled = false
        menu.addItem(header)
    ...
    private static let protectedApps: Set<String> = [
        ...
        "Lil EmergeX", "LilEmergeX", "Terminal", "iTerm2",
    ...
    func registerLaunchAtLogin() {
        ...
        let plistPath = "\(launchAgentDir)/dev.emergex.lil-emergex.plist"

        if !FileManager.default.fileExists(atPath: plistPath) {
            // Don't auto-enable, just make the option available
            print("[lil-emergex] Launch at login available via menu")
        }
    }

    func isLaunchAtLoginEnabled() -> Bool {
        let plistPath = NSString(string: "~/Library/LaunchAgents/dev.emergex.lil-emergex.plist").expandingTildeInPath
    ...
    @objc func toggleLaunchAtLogin() {
        ...
        if isLaunchAtLoginEnabled() {
            ...
            print("[lil-emergex] Removed launch at login")
        } else {
            ...
            <dict>
                <key>Label</key>
                <string>dev.emergex.lil-emergex</string>
            ...
            print("[lil-emergex] Enabled launch at login")
        }
...
class FileLogger {
    ...
    init() {
        let home = NSHomeDirectory()
        let dir = "\(home)/.emergex"
        try? FileManager.default.createDirectory(atPath: dir, withIntermediateDirectories: true)
        logPath = "\(dir)/lil-emergex.log"
        // Truncate on start
