#!/bin/bash
# Build Lil EmergeX - the EmergeX dock companion
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
BUILD_DIR="$SCRIPT_DIR/build"
APP_NAME="Lil EmergeX.app"
APP_DIR="$BUILD_DIR/$APP_NAME"

echo "Building Lil EmergeX v0.2.0..."

# 1. Generate sprites if missing
if [ ! -f "$SCRIPT_DIR/sprites/atlas.png" ]; then
  echo "Generating sprites..."
  cd "$SCRIPT_DIR/../.." && bun run apps/lil-emergex/generate-sprites.ts
fi
...
# 3. Copy Info.plist
cp "$SCRIPT_DIR/LilEmergeX/Info.plist" "$APP_DIR/Contents/"
...
swiftc \
  -o "$APP_DIR/Contents/MacOS/LilEmergeX" \
  -framework Cocoa \
  ...
  -O \
  "$SCRIPT_DIR/LilEmergeX/main.swift"
