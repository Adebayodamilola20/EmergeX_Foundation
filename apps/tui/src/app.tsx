/**
 * emergex Code - Main App Component
 *
...
import { SessionManager } from "../../../packages/emergex/session-manager.js";
import { SessionTree } from "../../../packages/emergex/session-tree.js";
...
import { Agent } from "../../../packages/emergex/index.js";
import type {
	AgentEvidenceEvent,
	...
	AgentToolEndEvent,
	AgentToolStartEvent,
} from "../../../packages/emergex/index.js";
...
function loadEnvFile() {
	// Check multiple locations: cwd first, then the emergex repo root
	const candidates = [
		pathMod.join(process.cwd(), ".env"),
		pathMod.resolve(import.meta.dirname, "../../../.env"), // emergex-code repo root
		pathMod.join(process.env.HOME || "", ".emergex", ".env"), // ~/.emergex/.env
	];
...
/** Merge saved ~/.emergex/providers.json with optional CLI --provider / --model. */
...
export function App({
	...
	useEffect(() => {
		if (viewMode === "splash") {
			const timer = setTimeout(() => {
				// Transition to either onboarding or chat after splash
	...
	useEffect(() => {
		const checkOnboarding = async () => {
			...
			if (!detected.githubUsername) {
				setMessages((prev) => [
					...prev,
					{
						...
						role: "assistant" as const,
						content:
							"Hey — you're not logged in to GitHub. Want me to log you in now so we can create issues, open PRs, and manage repos directly from here? Just say yes and I'll handle it.",
						timestamp: new Date(),
					},
				]);
			}
			...
			if (onboardingManager.needsOnboarding()) {
				...
				// Delay onboarding view if we are in splash
				if (viewMode !== "splash") {
					setViewMode("onboarding");
				}
	...
	const handleVisionCommand = useCallback(
		async (args: string[]) => {
			...
			} = await import("../../../packages/emergex/vision-router.js");
		...
		(command: SlashCommand, args: string[]) => {
			switch (command) {
				case "help":
					addSystemMessage(
						"Available commands:\n" +
							...
							"  /pet [start|stop|deck|card] - Lil EmergeX companion\n" +
					...
					if (sub === "status") {
						...
						addSystemMessage(
							"Telegram Bot Setup:\n\n" +
								...
								"   echo 'TELEGRAM_BOT_TOKEN=your_token_here' >> ~/.emergex/.env\n\n" +
								"5. Optionally restrict to your chat:\n" +
								"   - Message your bot, then check:\n" +
								"     curl https://api.telegram.org/bot<TOKEN>/getUpdates\n" +
								"   - Copy your chat_id and add:\n" +
								"     echo 'TELEGRAM_CHAT_ID=your_id' >> ~/.emergex/.env\n\n" +
								"6. Restart emergex to activate the bot.\n\n" +
								"Your bot becomes a mobile interface to emergex.",
						);
					} else {
						addSystemMessage(
							"Telegram commands:\n" +
				...
				case "export": {
					...
					const mins = Math.floor(elapsed / 60);
					const secs = elapsed % 60;
					import("../../../packages/emergex/session-export.js").then(
						({ saveSessionExport }) => {
							saveSessionExport(
								messages.map((m) => ({
									role: m.role,
						...
						.then(async ({ generateCompanion, formatDeckSummary }) => {
							...
							switch (petSub) {
								case "start": {
									...
									fs.mkdirSync(path.join(home, ".emergex"), {
										recursive: true,
									});
									fs.writeFileSync(
									...
									if (process.platform === "darwin") {
										// Try multiple paths: cwd (source), __dirname relative, home .emergex
										const candidates = [
											path.join(process.cwd(), "bin/lil-emergex.sh"),
											path.join(__dirname, "../bin/lil-emergex.sh"),
											path.join(__dirname, "../../bin/lil-emergex.sh"),
											path.join(
												process.env.HOME || "~",
												"emergex-code/bin/lil-emergex.sh",
											),
										];
										...
										if (lilEightScript) {
											...
											addSystemMessage(
												`[pet] lil-emergex.sh not found. Tried:\n  ${candidates.join("\n  ")}`,
											);
										}
									} else {
				...
				case "skills":
					void (async () => {
						try {
							...
							if (skills.length < 1) {
								addSystemMessage(
									"No skills loaded. Add .md under ~/.emergex/skills/ or run from a repo with .claude/skills/*/SKILL.md.",
								);
								return;
							}
							const lines = [
					...
					if ((command as string) === "auth") {
						...
						if (sub === "login") {
							...
							import("../../../packages/auth/cli-auth-server.js").then(
								({ runCLIAuthFlow }) => {
									runCLIAuthFlow("https://emergex.world", {
										onServerReady: () => {},
										onBrowserOpened: () => {},
										onWaiting: () => {},
																	...
																	.then((baseBranch) => {
																		...
																		tools
																			.createPR(token, info.owner, info.repo, {
																				title,
																				body: `Created via emergex Code from branch \`${branch}\`.`,
																				head: branch,
																				base: baseBranch,
																			})
																			.then((pr) => {
	...
	const handleSubmit = async (rawInput: string) => {
		...
		const isYes = /^(yes|yeah|yep|sure|ok|go ahead|do it|yup|y)$/i.test(
			input.trim(),
		);
		if (isGhAuthPrompt && isYes) {
			setMessages((prev) => [
				...
				{
					id: `gh-auth-running-${Date.now()}`,
					role: "assistant" as const,
					content: "Opening GitHub login in your browser...",
					timestamp: new Date(),
				},
			]);
			try {
			...
			case "splash":
				return (
					<Box
						...
						flexGrow={1}
						minHeight={viewport.height - 10}
					>
						<FancyHeader isProcessing={false} />
						<Box marginTop={1}>
							<MutedText>Initialising EmergeX Intelligence...</MutedText>
						</Box>
					</Box>
				);
