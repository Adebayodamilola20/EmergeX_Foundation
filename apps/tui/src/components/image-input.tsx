/**
 * emergex Code - Image Input Component
 *
 ...
 * Optimized to minimize fs.existsSync calls during typing.
 */
export function extractImagePaths(text: string): string[] {
  const whole = text.trim();
  if (!whole) return [];

  // FAST PATH: If the text doesn't look like a path or have an image extension, skip
  if (!whole.includes("/") && !whole.includes("\\") && !whole.startsWith("file:")) {
    return [];
  }

  // FAST PATH 2: If there's no hint of an image extension, skip
  if (!IMAGE_EXT_RE.test(whole)) {
    return [];
  }
  ...
  const tryToken = (token: string) => {
    ...
    // Be very strict about tokens to avoid fs checks on "normal" words
    if (!cleaned || cleaned.length < 4 || !IMAGE_EXT_RE.test(cleaned)) return;
    
    // Only check FS if it looks like an absolute path or relative starting with .
    if (!path.isAbsolute(cleaned) && !cleaned.startsWith(".") && !cleaned.includes("/") && !cleaned.includes("\\")) {
      return;
    }
  ...
  // 1. Try the whole trimmed string first (most common for drag-drop)
  tryToken(whole);
  if (paths.length > 0) return paths;

  // 2. Check for quoted paths (spaces inside)
  ...
  // 4. Whitespace / newline separated tokens (only if short string or has many slashes)
  // This is the most expensive part, so we limit it.
  if (text.length < 500 && (text.includes("/") || text.includes("\\"))) {
    const parts = text.split(/[\n\r\t]+|\s+/).filter(p => p.length > 4 && IMAGE_EXT_RE.test(p));
