/**
 * emergex Code - Chat Interface
 *
 * iMessage-style layout:
 * - User messages: right-aligned, yellow bubble
 * - emergex messages: left-aligned, cyan bubble
...
export function MessageList({
	...
	for (let i = collapsed.length - 1; i >= 0; i--) {
		const m = collapsed[i];
		const isPlan =
			m.content.includes("PLAN:") || m.content.includes("Plan detected");
...
function MessageItem({
	...
	const bubbleIndent = Math.max(
		2,
		Math.min(12, Math.floor(contentWidth * 0.1)),
	);
	...
	if (message.role === "tool") {
		...
		const icon = message.content.startsWith("  ✓")
			? "✓"
		...
		return (
			<Box marginBottom={1} paddingLeft={2}>
				...
				<MutedText>
					{message.content.replace(/^ {2}[✓✗→] /, "").slice(0, 100)}
				</MutedText>
			</Box>
		);
	}
		...
		>
			...
			<Box>
				{isUser ? (
					...
					<Label color="cyan">◆ emergex </Label>
				)}
				{isUser ? (
			...
			>
				<Box flexShrink={1} flexDirection="column" width={innerColumnWidth}>
					<MessageContent
