 ...
 * working directories. Reads from ~/.emergex/tabs/projects.json and scans
...
const SAVED_PROJECTS_PATH = join(homedir(), ".emergex", "tabs", "projects.json");
const SCAN_DIRS = [
	homedir(),
...
const PROJECT_MARKERS = ["package.json", ".git", "CLAUDE.md", ".emergex"];
...
function hasemergex(dirPath: string): boolean {
	return existsSync(join(dirPath, ".emergex"));
}
...
function scanForProjects(currentPath: string): ProjectEntry[] {
	...
	// Sort: hasemergex first, then by lastModified descending
	results.sort((a, b) => {
		if (a.path === currentPath) return -1;
		if (b.path === currentPath) return 1;
		if (a.hasemergex !== b.hasemergex) return a.hasemergex ? -1 : 1;
...
function saveSavedProjects(projects: SavedProject[]): void {
	try {
		const dir = join(homedir(), ".emergex", "tabs");
							...
							>
								<Stack gap={0}>
									...
									<Inline gap={1}>
										...
										{proj.hasemergex && (
											<Badge label="emergex" color="magenta" variant="outline" />
										)}
										{proj.gitBranch && (
