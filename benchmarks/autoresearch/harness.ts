#!/usr/bin/env bun
/**
 * emergex vs Claude Code - Autoresearch Benchmark Harness
 *
 * Following Karpathy's autoresearch methodology:
 * 1. Run benchmark on both Claude (baseline) and emergex
 * 2. Compare scores
 * 3. Modify emergex's prompts to close the gap
 * 4. Repeat until emergex surpasses baseline
 *
...
const PROMPTS_FILE = path.join(__dirname, "../../packages/emergex/prompts/system-prompt.ts");
...
async function getemergexSystemPrompt(): Promise<string> {
...
async function runBenchmarkOnemergex(task: BenchmarkTask): Promise<{ code: string; score: number }> {
  const fixture = fs.readFileSync(path.join(__dirname, "../..", task.fixture), "utf-8");
  const systemPrompt = await getemergexSystemPrompt();
...
function logResult(result: RunResult): void {
  const line = `${result.iteration}\t${result.benchmark}\t${result.claudeScore}\t${result.emergexScore}\t${result.gap}\t${result.status}\t${result.changes}\n`;
...
async function runAutoresearchLoop(): Promise<void> {
  ...
  console.log("  emergex vs Claude Code - Autoresearch Loop");
  ...
  while (true) {
    ...
    for (const task of BENCHMARKS) {
      ...
      try {
        const { code, score: emergexScore } = await runBenchmarkOnemergex(task);
        const gap = claudeScore - emergexScore;
[...]
        console.log(`   emergex score: ${emergexScore}`);
        console.log(`   Gap: ${gap > 0 ? "+" : ""}${gap} (${gap > 0 ? "Claude leads" : "emergex leads!"})`);

        let status: "improved" | "regressed" | "unchanged" = "unchanged";
        let changes = "none";

        if (emergexScore >= claudeScore) {
          iterationWins++;
          status = "improved";
          console.log(`   ✅ emergex WINS this benchmark!`);
        } else {
          // emergex lost - improve prompts
          console.log(`   ❌ emergex lost - improving prompts...`);
    ...
    console.log(`emergex wins: ${iterationWins}/${totalBenchmarks}`);
    console.log(`${"═".repeat(50)}`);

    // Check win condition
    if (iterationWins >= totalBenchmarks) {
      console.log("\n🎉 emergex has SURPASSED Claude Code on ALL benchmarks!");
...
export { runAutoresearchLoop, runBenchmarkOnemergex, gradeSolution };
