#!/usr/bin/env bun
/**
 * overnight-competition.ts — emergex vs Claude Code Overnight Competition
 *
 * Pits emergex (Ollama) against Claude (Anthropic API) on ALL benchmarks,
...
async function runEight(
  ...
  for (const temp of EIGHT_TEMPERATURES) {
    try {
      ...
      log(`    [emergex] temp=${temp} score=${result.score} (exec=${result.execution?.score ?? "-"} kw=${result.keyword.score}) ${durationMs}ms`);
      ...
      log(`    [emergex] temp=${temp} FAILED: ${err.message}`);
    }
  }
...
function analyzeAndMutate(
  ...
  if (claudeOnlyKeywords.length > 0) {
    newMutations.push(
      `[${benchmark.id}] Claude used patterns emergex missed: ${claudeOnlyKeywords.slice(0, 5).join(", ")}. Ensure these are present in your solution.`
    );
  }
  ...
  if (claudeExec && eightExec) {
    ...
    // Check if emergex timed out but Claude didn't
    if (eightExec.timedOut && !claudeExec.timedOut) {
      newMutations.push(
        `[${benchmark.id}] emergex's solution timed out. Use bounded iterations, avoid infinite loops, and ensure async operations complete.`
      );
    }
  }

  // If emergex failed extraction entirely
  if (!eightResult.rawOutput || eightResult.score === 0) {
    newMutations.push(
      `[${benchmark.id}] emergex produced no usable output. Ensure response contains properly fenced code blocks with \`\`\`typescript markers.`
    );
  }
...
async function runRound(
  ...
  for (const benchmark of category.benchmarks) {
    ...
    if (winner === "emergex") eightWins++;
    else if (winner === "Claude") claudeWins++;
    else ties++;

    log(`  RESULT: emergex=${eightResult.score} vs Claude=${claudeResult.score} => ${winner}`);
...
function formatTelegramRound(
  ...
  for (let i = 0; i < roundResult.eightResults.length; i++) {
    ...
    const winner =
      e.score > c.score ? "emergex" : c.score > e.score ? "Claude" : "TIE";
    perBenchmark += `${icon} ${e.benchmarkId}: emergex ${e.score} vs Claude ${c.score} [${winner}]\n`;
  }

  return `🏆 *EMERGEX vs CLAUDE — Round ${roundResult.round}*
⏰ ${time} PST

*Difficulty: ${roundResult.tier}*
📊 emergex: ${roundResult.eightAvg} | Claude: ${roundResult.claudeAvg}

Per-benchmark:
${perBenchmark.trim()}

*Mutations applied:* ${roundResult.mutationsApplied.length} new learnings
*Running score:* emergex ${totalEightWins} / Claude ${totalClaudeWins} / Ties ${totalTies}

_Next round in ~5m_`;
}

function formatFinalSummary(state: CompetitionState): string {
  const cats = state.rounds.map((r) => `${r.eightAvg >= r.claudeAvg ? "✅" : "❌"} ${r.category}: emergex ${r.eightAvg} vs Claude ${r.claudeAvg}`).join("\n");
  const muts = state.mutations.slice(-5).map((m) => `• ${m}`).join("\n") || "None";
  return `🏁 *EMERGEX vs CLAUDE — FINAL RESULTS*\n⏰ ${getPSTTimeString()} PST\n\n*Overall:* emergex ${state.totalEightWins} / Claude ${state.totalClaudeWins} / Ties ${state.totalTies}\n\n*Per-category:*\n${cats}\n\n*Top mutations:*\n${muts}\n\n*Learnings:* ${state.mutations.length} | *Rounds:* ${state.rounds.length}`;
}
    ...
    rounds: state.rounds.map((r) => ({
      ...
      benchmarks: r.eightResults.map((e, i) => {
        ...
        return { id: e.benchmarkId, eightScore: e.score, claudeScore: c.score,
          ...
          winner: e.score > c.score ? "emergex" : c.score > e.score ? "Claude" : "TIE" };
      }),
  ...
  return Object.entries(scores)
    .map(([id, d]) => {
      ...
      return { benchmarkId: id, category: d.cat, eightAvg: eAvg, claudeAvg: cAvg, gap: cAvg - eAvg,
        suggestion: `[GitHub Issue] emergex underperforms Claude by ${cAvg - eAvg}pts on ${id} (${d.cat}). emergex=${eAvg}, Claude=${cAvg}.` };
    })
    .filter((f) => f.gap >= 30)
...
function generateFinalReport(state: CompetitionState): void {
  ...
  for (const r of state.rounds) {
    const w = r.eightAvg >= r.claudeAvg ? "emergex" : "Claude";
    log(`    ${r.category.padEnd(25)} emergex=${String(r.eightAvg).padStart(3)} Claude=${String(r.claudeAvg).padStart(3)} => ${w}`);
  }
  ...
  if (failures.length > 0) {
    ...
    for (const f of failures) log(`    ⚠ ${f.benchmarkId}: gap=${f.gap} — emergex=${f.eightAvg} Claude=${f.claudeAvg}`);
...
async function main(): Promise<void> {
  ...
  log("╔══════════════════════════════════════════════════════════════╗");
  log("║     EMERGEX vs CLAUDE CODE — Overnight Competition           ║");
  log("╠══════════════════════════════════════════════════════════════╣");
  log(`║  emergex: ${EIGHT_MODEL} | Claude: ${claudeLabel}`);
  log(`║  Categories: ${CATEGORY_ROUNDS.length} | Benchmarks: ${totalBenchmarks} | Max rounds: ${MAX_ROUNDS || "all"}`);
  log(`║  Stop: ${STOP_HOUR}:00 PST | emergex temps: ${EIGHT_TEMPERATURES.join(",")} | Claude temp: ${CLAUDE_TEMPERATURE}`);
  log("╚══════════════════════════════════════════════════════════════╝\n");
  ...
  await sendTelegram(`🚀 *EMERGEX vs CLAUDE — Started*\n⏰ ${getPSTTimeString()} PST\nemergex: \`${EIGHT_MODEL}\` | Claude: \`${claudeLabel}\`\nCategories: ${CATEGORY_ROUNDS.length} | Round: ${state.currentRound + 1}`);
  ...
  for (let i = startRound; i < endRound; i++) {
    ...
    // Log round summary
    log(`\n  Round ${i + 1} Summary: emergex avg=${roundResult.eightAvg} Claude avg=${roundResult.claudeAvg}`);
    log(`  Round wins: emergex=${roundResult.eightWins} Claude=${roundResult.claudeWins} Ties=${roundResult.ties}`);
    log(`  Cumulative: emergex=${state.totalEightWins} Claude=${state.totalClaudeWins} Ties=${state.totalTies}`);
