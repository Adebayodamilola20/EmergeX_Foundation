...
#   Phase 2 (1:00 AM - 4:00 AM):  Claude vs emergex head-to-head on ALL categories
...
#   Phase 5 (6:00 AM - 6:30 AM):  Sync results to emergex-world, generate summary
...
⚠️ *emergex Overnight Competition Interrupted*
...
log_header "EMERGEX OVERNIGHT COMPETITION — $(date '+%Y-%m-%d')"
PHASE="setup"
...
send_telegram "$(cat <<MSG
🏁 *emergex Overnight Competition Started*
...
# ══════════════════════════════════════════════════════════════════════
# PHASE 2: Head-to-Head — Claude vs emergex, ALL categories (1:00 AM - 4:00 AM)
# ══════════════════════════════════════════════════════════════════════

PHASE="phase2-headtohead"
log_header "PHASE 2: Head-to-Head — Claude vs emergex (ALL categories)"
...
EOF
  ...
  if [ -f "$STATE_FILE" ]; then
    H2H_WINS=$(jq -r '[.results[].winner // empty] | map(select(. == "emergex")) | length' "$STATE_FILE" 2>/dev/null || echo "?")
...
MSG
  ...
  log "Phase 2 complete. emergex: ${H2H_WINS}W / ${H2H_LOSSES}L / ${H2H_TIES}T (${H2H_TOTAL} total)"
fi
...
# ══════════════════════════════════════════════════════════════════════
# PHASE 5: Sync — Push results to emergex-world (6:00 AM - 6:30 AM)
# ══════════════════════════════════════════════════════════════════════

PHASE="phase5-sync"
log_header "PHASE 5: Sync results to emergex-world"
...
send_telegram "$(cat <<MSG
🏆 *emergex Overnight Competition Complete*
...
• H2H: emergex ${H2H_WINS:-?}W / Claude ${H2H_LOSSES:-?}W / ${H2H_TIES:-?}T
