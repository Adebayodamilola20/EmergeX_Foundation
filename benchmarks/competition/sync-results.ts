#!/usr/bin/env bun
/**
 * sync-results.ts — Sync overnight competition results to emergex-world
 *
 * Reads competition + autoresearch state, generates summary,
 * updates emergex-world benchmark pages, and optionally creates a GitHub gist.
 */
...
const EIGHT_WORLD = path.join(process.env.HOME ?? "~", "emergex-world");
...
function buildSummary(): SyncSummary {
  ...
  const eightWins = results.filter(r => r.winner === "emergex").length;
...
// ── Update emergex-world benchmarking page ──────────────────────────────

function updateBenchmarkingPage(summary: SyncSummary): boolean {
  if (!fs.existsSync(BENCHMARKING_PAGE)) {
    console.log("emergex-world benchmarking page not found — skipping");
    return false;
  }
...
function commitToWorld(summary: SyncSummary): boolean {
  ...
  try {
    ...
    if (!status.trim()) {
      console.log("No changes to commit in emergex-world");
      return false;
    }

    execSync(`git commit -m "${msg}"`, { cwd: EIGHT_WORLD, stdio: "pipe" });
    console.log(`Committed to emergex-world: ${msg.split("\n")[0]}`);
    return true;
  } catch (err) {
    console.error("Failed to commit to emergex-world:", err);
    return false;
  }
}
...
function createGist(summary: SyncSummary): string | null {
  ...
  try {
    ...
    const tmpFile = path.join(REPO_ROOT, `.emergex/competition-summary-${summary.date}.json`);
    fs.mkdirSync(path.dirname(tmpFile), { recursive: true });
    fs.writeFileSync(tmpFile, gistContent);

    const result = execSync(
      `gh gist create "${tmpFile}" --desc "emergex overnight competition results ${summary.date}" --public`,
...
function formatSummary(summary: SyncSummary): string {
  ...
  lines.push("═══════════════════════════════════════════════════════════");
  lines.push(`  emergex Overnight Competition Summary — ${summary.date}`);
  ...
  lines.push(`    emergex wins:  ${h2h.eightWins}`);
...
async function main() {
  ...
  const summaryPath = path.join(REPO_ROOT, `.emergex/competition-summary-${summary.date}.json`);
  fs.mkdirSync(path.dirname(summaryPath), { recursive: true });
  fs.writeFileSync(summaryPath, JSON.stringify(summary, null, 2));
  console.log(`\nSaved summary: ${summaryPath}`);

  // Update emergex-world if it exists
  if (fs.existsSync(EIGHT_WORLD)) {
    console.log("\nSyncing to emergex-world...");
    ...
    if (pageUpdated || sceneUpdated) {
      ...
      if (committed) {
        console.log("Changes committed to emergex-world");
      }
    }
  } else {
    console.log("\n~/emergex-world not found — skipping sync");
  }
