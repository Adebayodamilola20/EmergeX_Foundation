#!/usr/bin/env bun
/**
 * emergex vs Claude — Head-to-Head SWE Benchmark
 *
 * Runs identical coding tasks against both:
 * - emergex (eight:latest via Ollama bridge)
...
const RESULTS_DIR = path.join(import.meta.dir, "../.emergex/benchmarks");
...
// Load the full system prompt from emergex's prompt library
let cachedSystemPrompt: string | null = null;
function getEightSystemPrompt(): string {
  if (cachedSystemPrompt) return cachedSystemPrompt;
  try {
    const promptFile = path.join(import.meta.dir, "../packages/emergex/prompts/system-prompt.ts");
...
async function runemergex(task: SWETask): Promise<{ output: string; durationMs: number }> {
  ...
  if (!res.ok) throw new Error(`emergex Ollama error: ${res.status}`);
...
{"scoreA": <number>, "scoreB": <number>, "analysisA": "<one sentence>", "analysisB": "<one sentence>", "winner": "A" | "B" | "tie"}`;
  ...
  try {
    ...
    return {
      ...
      winner: parsed.winner === "A" ? "emergex" : parsed.winner === "B" ? "Claude" : "tie",
    };
  } catch {
...
async function main() {
  console.log("\n╔══════════════════════════════════════════════════════════╗");
  console.log("║     emergex vs Claude — Head-to-Head SWE Benchmark       ║");
  console.log("╚══════════════════════════════════════════════════════════╝\n");
  console.log(`  emergex model:  ${EIGHT_MODEL}`);
  ...
  for (const task of TASKS) {
    ...
    console.log("  Running emergex...");
    let eightResult: { output: string; durationMs: number };
    try {
      eightResult = await runemergex(task);
      console.log(`  ✓ emergex: ${eightResult.durationMs}ms, ${eightResult.output.length} chars`);
    } catch (err: any) {
      console.log(`  ✗ emergex failed: ${err.message}`);
    ...
    if (verdict.winner === "emergex") eightWins++;
    else if (verdict.winner === "Claude") claudeWins++;
    else ties++;

    const icon = verdict.winner === "emergex" ? "🏆" : verdict.winner === "Claude" ? "🥈" : "🤝";
    console.log(`  ${icon} Winner: ${verdict.winner}`);
    console.log(`     emergex: ${verdict.eightScore}/100 — ${verdict.eightAnalysis}`);
  ...
  console.log(`║  emergex  Wins: ${eightWins}  |  Avg Score: ${eightAvg.toFixed(1)}/100`.padEnd(59) + "║");
  console.log(`║  Claude Wins: ${claudeWins}  |  Avg Score: ${claudeAvg.toFixed(1)}/100`.padEnd(59) + "║");
  console.log(`║  Ties:        ${ties}`.padEnd(59) + "║");
  console.log("╠══════════════════════════════════════════════════════════╣");

  for (const r of results) {
    const icon = r.winner === "emergex" ? "🏆" : r.winner === "Claude" ? "🥈" : "🤝";
    console.log(`║  ${icon} ${r.task.id}: emergex ${r.eightScore} vs Claude ${r.claudeScore} (${r.winner})`.padEnd(59) + "║");
  }

  const overall = eightWins > claudeWins ? "🏆 EMERGEX WINS" : claudeWins > eightWins ? "🥈 CLAUDE WINS" : "🤝 TIE";
  console.log("╠══════════════════════════════════════════════════════════╣");
