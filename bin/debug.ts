#!/usr/bin/env bun
/**
 * emergex Debug CLI — inspect, tail, and analyze TUI sessions.
 *
...
async function cmdHealth(): Promise<void> {
  console.log(`${c.boldCyan}emergex Health Check${c.reset}\n`);
  ...
  const authPath = path.join(DOT_EMERGEX, "auth.json");
  ...
  // 6. Disk usage of ~/.emergex
  process.stdout.write(`  ~/.emergex size: `);
  try {
    const proc = Bun.spawn(["du", "-sh", DOT_EMERGEX], {
      stdout: "pipe",
      stderr: "pipe",
    });
...
function usage(): void {
  console.log(`${c.boldCyan}emergex Debug CLI${c.reset}
