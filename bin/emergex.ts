#!/usr/bin/env bun
/**
 * emergex Code - Main CLI Entry Point
 *
...
const HELP = `
EmergeX Code v${VERSION}

USAGE:
  emergex <command> [options]
...
COMMANDS:
  ...
  pet                         Launch Lil EmergeX dock companion only
  ...
  init                        Initialize emergex in current directory
...
OPTIONS:
  ...
  --pet          Also spawn Lil EmergeX dock companion (auto with tui)
  --no-pet       Disable dock pet auto-spawn
  --rpc          Start in JSON-RPC mode (headless, stdin/stdout)

EXAMPLES:
  # Non-interactive chat (pipe-friendly)
  emergex chat "Fix the auth middleware" --json
  echo "Explain this code" | emergex chat --stdin

  # Multi-agent orchestration
  emergex agent spawn winston "Design the multi-tenant schema"
  emergex agent list --json
  emergex agent message winston-1 "Focus on data isolation"

  # Session management
  emergex session list --json --limit=5
  emergex session resume sess_abc123

  # Preferences
  emergex preferences get --json
  emergex preferences set model qwen3:14b

  # Memory
  emergex memory recall "auth middleware" --json
  emergex memory remember "Uses JWT with RS256 for auth"

  # Autonomous execution
  emergex infinite "Build a REST API for user management"
  emergex chat "Add tests" --yes --infinite
...
Learn more: https://github.com/8gi-foundation/emergex-code
`;
...
async function main() {
  ...
  if (args.includes("--rpc")) {
    const { startRPCServer } = await import("../packages/emergex/rpc-server.ts");
  ...
  if (args.includes("--cli")) {
    const { runCLI } = await import("../packages/emergex/cli.ts");
    await runCLI(args);
    return;
  }
  ...
  if (args.includes("-v") || args.includes("--version")) {
    console.log(`EmergeX Code v${VERSION}`);
    return;
  }
  ...
  // Handle --pet / -pet as standalone (same as emergex pet start)
  ...
  // Implicit TUI: `emergex --provider=lmstudio` so global flags reach the TUI child process
  ...
  switch (command) {
    ...
    default:
      ...
      console.log(`Run 'emergex --help' for usage information.`);
      process.exit(1);
  }
}
...
async function initCommand(args: string[]) {
  console.log(BANNER);
  console.log("Initializing EmergeX Code...\n");

  const configPath = path.join(process.cwd(), ".emergex");
  ...
  if (!fs.existsSync(configPath)) {
    ...
    console.log("✅ Created .emergex directory");
    console.log("✅ Created config.json");
    console.log("\n🚀 EmergeX Code is ready!");
    console.log("   Run 'emergex outline <file>' to get started.");
  } else {
    console.log("EmergeX is already initialized in this directory.");
  }
}

async function outlineCommand(args: string[]) {
  if (args.length === 0) {
    console.error("Usage: emergex outline <file>");
    process.exit(1);
  }
...
async function symbolCommand(args: string[]) {
  if (args.length === 0) {
    console.error("Usage: emergex symbol <file>::<symbolName>");
    process.exit(1);
  }
...
async function searchCommand(args: string[]) {
  if (args.length === 0) {
    console.error("Usage: emergex search <query> [--kinds=function,class] [--dir=src/]");
    process.exit(1);
  }
...
// Spawn Lil EmergeX dock pet (non-blocking, idempotent)
async function spawnPet(sessionId?: string) {
  ...
  if (platform !== "darwin") {
    console.log("\x1b[36m[pet] Lil EmergeX terminal mode (cross-platform)\x1b[0m");
    ...
    try {
      ...
      if (fs.existsSync(petPath)) {
        ...
        const pet = new TerminalPet({ label: sessionId || "emergex" });
    ...
    try {
      ...
      const meshDir = path.join(home, ".emergex", "mesh");
      ...
      const agentId = sessionId || `emergex-tui-${process.pid}`;
      registry[agentId] = {
        id: agentId, type: "emergex", name: "TUI", pid: process.pid,
  ...
  try {
    ...
    fs.mkdirSync(path.join(home, ".emergex"), { recursive: true });
    fs.writeFileSync(path.join(home, ".emergex", "active-companion.json"), JSON.stringify({
  ...
  const lilEmergeXScript = path.join(__dirname, "lil-emergex.sh");

  // Kill existing pets, then spawn fresh
  try { execSync("pkill -f LilEmergeX 2>/dev/null", { stdio: "pipe" }); } catch {}

  // Spawn new pet
  {
    // Not running - spawn it
    if (fs.existsSync(lilEmergeXScript)) {
      const pet = spawnProc("bash", [lilEmergeXScript, "start"], {
        detached: true,
        stdio: "ignore",
      });
      pet.unref();
      console.log("\x1b[36m[pet] Lil EmergeX spawned on Dock\x1b[0m");
    } else {
      // Try build first
      const buildScript = path.join(__dirname, "../apps/lil-emergex/build.sh");
      if (fs.existsSync(buildScript)) {
        console.log("\x1b[36m[pet] Building Lil EmergeX...\x1b[0m");
        try {
          ...
          console.log("\x1b[33m[pet] Could not build Lil EmergeX\x1b[0m");
        }
      }
    }
  }
  ...
  try {
    const meshDir = path.join(process.env.HOME || "~", ".emergex", "mesh");
    const registryPath = path.join(meshDir, "registry.json");
    fs.mkdirSync(meshDir, { recursive: true });
...
async function airdropCommand(args: string[]) {
  if (process.platform !== "darwin") {
    console.log("AirDrop is macOS only. Use emergex send for cross-platform sharing.");
    return;
  }

  if (args.length === 0) {
    console.log("Usage: emergex airdrop <file|text>");
    console.log("  emergex airdrop ./report.pdf     - AirDrop a file");
    console.log("  emergex airdrop \"Hello from emergex\" - AirDrop text as a note");
    console.log('  emergex drop ./file.txt          - shorthand');
    return;
  }
  ...
  if (fs.existsSync(target) || fs.existsSync(path.resolve(target))) {
    ...
    const tmpSwift = "/tmp/emergex-airdrop.swift";
    fs.writeFileSync(tmpSwift, swiftHelper);
    try {
      execSync(`swiftc -o /tmp/emergex-airdrop -framework Cocoa "${tmpSwift}" && /tmp/emergex-airdrop`, {
        stdio: "inherit",
        timeout: 60000,
      });
    } catch {
    ...
    const tmpFile = `/tmp/emergex-note-${Date.now()}.txt`;
...
async function petCommand(args: string[]) {
  ...
  if (!fs.existsSync(lilEmergeXScript)) {
    console.log("Building Lil EmergeX first...");
    const buildScript = path.join(__dirname, "../apps/lil-emergex/build.sh");
  ...
  switch (subCmd) {
    ...
    case "stop":
    case "kill":
      try { execSync("pkill -f LilEmergeX"); console.log("Lil EmergeX stopped"); } catch { console.log("Not running"); }
      break;
    case "restart":
    ...
    case "logs":
      ...
      const logPath = path.join(process.env.HOME || "~", ".emergex", "lil-emergex.log");
      sp("tail", ["-f", logPath], { stdio: "inherit" });
      break;
    case "status":
      try {
        execSync("pgrep -f LilEmergeX", { stdio: "pipe" });
        console.log("Lil EmergeX is running");
        const logP = path.join(process.env.HOME || "~", ".emergex", "lil-emergex.log");
        if (fs.existsSync(logP)) {
          const lines = fs.readFileSync(logP, "utf-8").trim().split("\n").slice(-3);
          lines.forEach(l => console.log(`  ${l}`));
        }
      } catch { console.log("Lil EmergeX is not running"); }
      break;
    default:
      console.log("Usage: emergex pet [start|stop|restart|build|log|status]");
  }
}
...
async function infiniteCommand(args: string[]) {
  ...
  if (args.length === 0) {
    console.error("Usage: emergex infinite <task description>");
    console.log('\nExample: emergex infinite "Build a Next.js landing page with dark theme"');
    process.exit(1);
  }
...
async function authCommand(args: string[]) {
  ...
  switch (subcommand) {
    case "login": {
      console.log("Authenticating with emergex...\n");
    ...
    case "status": {
      ...
      if (state.state === "authenticated") {
        ...
        console.log("Run `emergex auth login` to authenticate with GitHub.");
    ...
    default:
      console.log(`
emergex auth — Authentication Commands

USAGE:
  emergex auth <subcommand>
...
async function cronCommand(args: string[]) {
  ...
  if (!sub || sub === "list") {
    const jobs = mgr.list();
    if (jobs.length === 0) {
      console.log("No cron jobs configured. Use: emergex cron add <name> <schedule> <command>");
      return;
    }
    ...
    if (args.length < 4) {
      console.error("Usage: emergex cron add <name> <schedule> <command>");
      console.error('Example: emergex cron add "deploy-check" "0 */6 * * *" "chat \'check deploy status\'"');
      process.exit(1);
    }
    const name = args[1];
    ...
    if (!args[1]) { console.error("Usage: emergex cron remove <id>"); process.exit(1); }
    const removed = mgr.remove(args[1]);
    console.log(removed ? `Removed job ${args[1]}` : `Job ${args[1]} not found`);
  } else if (sub === "enable") {
    if (!args[1]) { console.error("Usage: emergex cron enable <id>"); process.exit(1); }
    mgr.enable(args[1]);
    console.log(`Enabled job ${args[1]}`);
  } else if (sub === "disable") {
    if (!args[1]) { console.error("Usage: emergex cron disable <id>"); process.exit(1); }
    mgr.disable(args[1]);
...
async function chatCommand(args: string[]) {
  ...
  if (!message) {
    console.error("Usage: emergex chat <message> [--json] [--model=<m>] [--stdin]");
    process.exit(1);
  }
  ...
  try {
    const { Agent } = await import("../packages/emergex/agent");
...
async function agentCommand(args: string[]) {
  ...
  switch (sub) {
    ...
    case "spawn": {
      ...
      if (!personaId) {
        ...
        if (isJson) {
          ...
          console.log("Usage: emergex agent spawn <persona> <task>\n");
    ...
    case "kill": {
      const killId = rest[1];
      if (!killId) {
        console.error("Usage: emergex agent kill <agent-id>");
        process.exit(1);
      }
    ...
    case "message": {
      ...
      if (!targetId || !msg) {
        console.error("Usage: emergex agent message <agent-id> <message>");
        process.exit(1);
      }
...
async function sessionCommand(args: string[]) {
  ...
  const isJson = !!flags.json;
  const sub = rest[0] || "list";
  const { SessionManager } = await import("../packages/emergex/session-manager");
  ...
  switch (sub) {
    ...
    case "resume": {
      ...
      if (!nameOrId) {
        console.error("Usage: emergex session resume <name-or-id>");
        process.exit(1);
      }
...
async function exportCommand(args: string[]) {
  ...
  if (!sessionId && !isLast) {
    console.error("Usage: emergex export <session-id> or emergex export --last");
    process.exit(1);
  }

  const { SessionSyncManager } = await import("../packages/emergex/session-sync");
  ...
  const { saveSessionExport } = await import("../packages/emergex/session-export");
...
async function preferencesCommand(args: string[]) {
  ...
  switch (sub) {
    ...
    case "set": {
      ...
      if (!key || !value) {
        console.error("Usage: emergex preferences set <key> <value>");
    ...
    case "sync": {
      ...
      try {
        ...
        if (authState.state !== "authenticated") {
          console.error("Not authenticated. Run `emergex auth login` first.");
          process.exit(2);
        }
...
async function memoryCommand(args: string[]) {
  ...
  switch (sub) {
    case "recall": {
      ...
      if (!query) {
        console.error("Usage: emergex memory recall <query>");
        process.exit(1);
      }
    ...
    case "remember": {
      ...
      if (!fact) {
        console.error("Usage: emergex memory remember <fact>");
        process.exit(1);
      }
    ...
    case "forget": {
      const id = rest[1];
      if (!id) {
        console.error("Usage: emergex memory forget <id>");
        process.exit(1);
      }
...
async function statusCommand(args: string[]) {
  ...
  // Check .emergex config
  try {
    const configPath = path.join(cwd, ".emergex", "config.json");
    if (fs.existsSync(configPath)) {
...
async function doctorCommand() {
  ...
  const dbPath = path.join(process.env.HOME || "", ".emergex", "memory.db");
  console.log(fs.existsSync(dbPath) ? green(`Memory DB (${(fs.statSync(dbPath).size/1024).toFixed(0)} KB)`) : dim("  No memory DB yet"));
  const cfgPath = path.join(process.env.HOME || "", ".emergex", "config.json");
  ...
  const mcpPath = path.join(process.env.HOME || "", ".emergex", "mcp.json");
  ...
  else console.log(dim("  No MCP servers (~/.emergex/mcp.json)"));
