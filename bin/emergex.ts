...
async function main() {
	...
	if (args.includes("--rpc")) {
		const { startRPCServer } = await import(
			"../packages/emergex/rpc-server.ts"
		);
...
async function searchCommand(args: string[]) {
	if (args.length === 0) {
		console.error(
			"Usage: emergex search <query> [--kinds=function,class] [--dir=src/]",
		);
		process.exit(1);
	}
...
async function spawnPet(sessionId?: string) {
	...
	if (platform !== "darwin") {
		console.log(
			"\x1b[36m[pet] Lil EmergeX terminal mode (cross-platform)\x1b[0m",
		);
	...
	try {
		...
		fs.writeFileSync(
			path.join(home, ".emergex", "active-companion.json"),
			JSON.stringify(
				{
	...
	try {
		execSync("pkill -f LilEmergeX 2>/dev/null", { stdio: "pipe" });
	} catch {}
...
async function airdropCommand(args: string[]) {
	if (process.platform !== "darwin") {
		console.log(
			"AirDrop is macOS only. Use emergex send for cross-platform sharing.",
		);
		return;
	}
	...
	if (args.length === 0) {
		...
		console.log(
			'  emergex airdrop "Hello from emergex" - AirDrop text as a note',
		);
    ...
    `;
		...
		try {
			execSync(
				`swiftc -o /tmp/emergex-airdrop -framework Cocoa "${tmpSwift}" && /tmp/emergex-airdrop`,
				{
					stdio: "inherit",
					timeout: 60000,
				},
...
async function petCommand(args: string[]) {
	...
	switch (subCmd) {
		...
		case "kill":
			try {
				execSync("pkill -f LilEmergeX");
				console.log("Lil EmergeX stopped");
			} catch {
		...
		case "collection": {
			...
			if (deck.companions.length === 0) {
				console.log(
					"Your companion deck is empty. Start a session to spawn your first friend!",
				);
				return;
			}
			...
			for (const entry of deck.companions) {
				...
				const shiny = c.shiny ? "🌟 " : "";
				console.log(
					`${shiny}${c.fullName} — ${c.rarity} ${c.species} (${c.element})`,
				);
				console.log(`   Title: ${c.title} • Accessory: ${c.accessory}`);
				console.log(
					`   Stats: DBG ${c.stats.debug} CHA ${c.stats.chaos} WIS ${c.stats.wisdom} PAT ${c.stats.patience}`,
				);
			}
			break;
		}
		case "build":
			execSync(
				`bash "${path.join(__dirname, "../apps/lil-emergex/build.sh")}"`,
				{ stdio: "inherit" },
			);
			break;
		case "log":
		...
		case "status":
			try {
				...
				console.log("Lil EmergeX is not running");
			}
			break;
		default:
...
async function cronCommand(args: string[]) {
	...
	if (!sub || sub === "list") {
		...
		if (jobs.length === 0) {
			console.log(
				"No cron jobs configured. Use: emergex cron add <name> <schedule> <command>",
			);
			return;
		}
		...
		if (args.length < 4) {
			...
			console.error(
				'Example: emergex cron add "deploy-check" "0 */6 * * *" "chat \'check deploy status\'"',
			);
			process.exit(1);
		}
		const name = args[1];
		...
		if (!args[1]) {
			console.error("Usage: emergex cron remove <id>");
			process.exit(1);
		}
		...
		if (!args[1]) {
			console.error("Usage: emergex cron enable <id>");
			process.exit(1);
		}
		mgr.enable(args[1]);
		...
		if (!args[1]) {
			console.error("Usage: emergex cron disable <id>");
			process.exit(1);
		}
		mgr.disable(args[1]);
...
async function chatCommand(args: string[]) {
	...
	if (!message) {
		console.error(
			"Usage: emergex chat <message> [--json] [--model=<m>] [--stdin]",
		);
		process.exit(1);
	}
...
async function sessionCommand(args: string[]) {
	...
	const { SessionManager } = await import(
		"../packages/emergex/session-manager"
	);
...
async function exportCommand(args: string[]) {
	...
	if (!sessionId && !isLast) {
		console.error(
			"Usage: emergex export <session-id> or emergex export --last",
		);
		process.exit(1);
	}

	const { SessionSyncManager } = await import(
		"../packages/emergex/session-sync"
	);
	...
	const { saveSessionExport } = await import(
		"../packages/emergex/session-export"
	);
