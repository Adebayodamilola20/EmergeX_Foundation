...
emergex uses Karpathy's autoresearch methodology to iteratively improve its system prompts by running benchmarks in a loop and automatically enhancing weak areas.
...
1. **Extracts the system prompt** from `packages/emergex/prompts/system-prompt.ts`
...
5. **Generates enhanced patterns** for categories where emergex underperforms
