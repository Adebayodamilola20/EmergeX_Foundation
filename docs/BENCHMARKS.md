...
emergex includes a comprehensive benchmark suite with 44 tests across 12 categories, designed to evaluate coding agent performance across diverse domains.
...
```
iteration	benchmark_id	claude_baseline	emergex_score	gap	status	action
...
The benchmark suite doubles as a **regression gate** for RL fine-tuning checkpoints. When the `@emergex/kernel` package trains a new LoRA checkpoint via the training proxy, it validates the checkpoint against the benchmark suite before promotion.
...
2. Validation script runs benchmark suite through training proxy (`:30000`)
