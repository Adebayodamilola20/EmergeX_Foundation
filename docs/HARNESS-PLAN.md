...
Concrete implementation plan for inheriting 8 abilities into the emergex-code vessel harness.
...
| Gap | FoodstackOS Pattern | emergex-code Status |
|-----|---------------------|-------------------|
...
| `packages/emergex/tools.ts` | Modify | ~40 | Add 4 new tools: browser_open, browser_state, browser_task, browser_screenshot |
...
- `packages/emergex/prompts/system-prompt.ts` - Single system prompt with USER_CONTEXT_SEGMENT injection
...
| File | Action | Lines | Description |
|------|--------|-------|-------------|
| `packages/emergex/prompts/soul-layers.ts` | Create | ~150 | Access tier definitions, prompt segment composer |
| `packages/emergex/prompts/system-prompt.ts` | Modify | ~30 | Replace monolithic prompt with layered composition |
...
`packages/emergex/tools.ts` - 24 tools in a flat list:
...
| File | Action | Lines | Description |
|------|--------|-------|-------------|
| `packages/emergex/tools.ts` | Modify | ~100 | Rewrite all 24 tool descriptions for model consumption |
| `packages/emergex/prompts/system-prompt.ts` | Modify | ~20 | Add tool usage patterns section |
