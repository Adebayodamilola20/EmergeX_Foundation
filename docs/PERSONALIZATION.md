...
emergex learns who you are, how you work, and what you prefer. Authenticate once, never configure again. Every session gets smarter.
...
You can skip individual questions with `/skip` or skip everything with `/skip all`. Skipped areas are tracked in `understanding.areasUnclear` and emergex may ask follow-up clarification questions later (after 7 days or when confidence drops below 0.8).
...
emergex personalizes its system prompt based on your onboarding data. The `USER_CONTEXT_SEGMENT` is injected into every agent turn.
...
emergex collects high-quality session traces for personal LoRA fine-tuning via the `PersonalCollector`.
...
Collection happens automatically. To enable the full RL training pipeline (GRPO batches, checkpoint validation, auto-promotion), set in `.emergex/config.json`:
...
```json
{
  ...
  "projects": {
    "primary": "emergex-code",
    "all": ["emergex-code", "foodstack"],
    "descriptions": {
      "emergex-code": "Autonomous coding agent TUI"
    }
  },
...
### `.emergex/config.json` Sync Flags
...
The `understanding.confidenceScore` (0.0--1.0) represents how well emergex knows you. It is calculated from five equally-weighted categories:
...
When confidence drops below 0.8, or 7 days pass since the last prompt, emergex may ask a follow-up clarification question to fill in gaps.
