...
**Build:**
- `packages/emergex/tool-gate.ts` — middleware that wraps every tool call
...
**Build:**
- `packages/emergex/github-provider.ts` — tiny surface area:
  - `createBranch()`, `commit()`, `push()`, `openPR()`
  - `createIssueFromFailure(benchmark, error, score)`
  - `postBenchmarkComment(prNumber, results)`
- `packages/emergex/benchmark-reporter.ts` — emits normalized result object, GitHubProvider posts it
...
**Build:**
- `packages/emergex/checkpoint.ts` — minimal:
...
- `packages/emergex/post-run-review.ts` — after each task:
...
**Build:**
- `packages/emergex/hypothesis.ts` — minimal:
  ...
  - Lessons file: append to `~/.emergex/lessons.jsonl`
