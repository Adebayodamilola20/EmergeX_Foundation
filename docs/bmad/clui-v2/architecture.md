...
> **Eight = kernel. emergex-code = client. CLUI = the rich desktop surface.**
...
```
  ...
  |                        Monorepo Packages                         |
  |                                                                  |
  |  @emergex/voice     @emergex/planning   @emergex/validation            |
  |  (Whisper STT)    (ProactivePlanner  (EvidenceCollector           |
  |                    AvenueTracker)     ValidationReporter)         |
  |                                                                  |
  |  @emergex/auth      @emergex/eight       @emergex/kernel               |
...
- Loads system prompt from `packages/emergex/prompts/system-prompt.ts` (falls back to inline default)
...
There is also a secondary bridge at `packages/emergex/clui-bridge.ts` with identical logic. The standalone `apps/clui/bridge.ts` is preferred at runtime to avoid workspace import issues.
...
| `preferences-store.ts` | `theme`, `activeModel`, `savedModels`, `showEvidencePanel`, `showKanban`, `usage` | localStorage (`emergex-clui-preferences`) |
| `auth-store.ts` | `stateName`, `user`, `deviceCode`, `isAuthenticated` | Keychain via `@emergex/auth` |
...
**`useAuthStore`** -- Persisted to OS Keychain via `@emergex/auth`.
...
- Lazy-loads `@emergex/auth` to avoid blocking app startup
...
```
      ...
      +--------+-------+  +-------+--------+  +-------+--------+
               |                   |                   |
               v                   v                   v
          /tmp/*.wav          ~/.emergex/models/     processLevel()
...
- WAV files: OS temp directory (`/tmp/emergex-voice-*.wav`)
- Auto-deleted after transcription
- Whisper models: `~/.emergex/models/` (auto-downloaded on first use)
...
The `ProactivePlanner` and `AvenueTracker` from `@emergex/planning` drive surfacing decisions:
...
`@emergex/voice`, `@emergex/planning`, and `@emergex/validation` are separate packages, not bundled into the bridge. This means:
...
**Tradeoff:** The bridge currently only does chat completion. Tool execution, evidence collection, and planning must be wired through additional IPC channels or by replacing the bridge with the full `@emergex/eight` engine. This is the primary v2 work item.
...
- Tokens persisted in OS Keychain via `@emergex/auth`
...
1. **Bridge evolution:** Should the bridge be replaced with the full `@emergex/eight` engine (which has tool execution, planning, evidence collection)? Or should those capabilities be added incrementally via new NDJSON event types?
