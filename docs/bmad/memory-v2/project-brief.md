...
The current emergex memory system (`packages/memory/index.ts`, ~450 lines) is a minimal v1 prototype with fundamental limitations that prevent emergex from becoming a true AI OS kernel:
...
Without intelligent memory, every emergex session starts nearly from scratch. The agent repeatedly asks the same questions, cannot learn from past mistakes, cannot build a knowledge graph of the codebase, and cannot transfer learnings between projects. This is the single largest gap between emergex and a production-grade AI OS kernel.

---

## 2. Vision

Memory v2 transforms emergex from a stateless coding tool into a continuously learning AI collaborator. The system should:
...
> An emergex instance that has worked on a project for 20 sessions should produce measurably better first-turn responses than a fresh instance, as judged by an LLM evaluator (not string matching).
...
| FoodstackOS Feature | emergex Adaptation |
|---------------------|-----------------|
...
|---------------------|----------|
| **Multi-tenant isolation** (organizationId boundaries) | emergex is single-user; project-level scoping is sufficient |
| **OpenAI embeddings** (text-embedding-3-small, 1536-dim, $0.02/1M tokens) | emergex is local-first; use Ollama nomic-embed-text (768-dim, free, offline) |
| **Convex as primary store** | emergex needs local SQLite for offline operation; Convex is optional sync target |
...
- [ ] **Migration from v1** — import existing `.emergex/memory/project.jsonl` and `~/.emergex/memory/global.jsonl`
...
- [ ] **Memory federation** — share memory subsets between emergex instances (multi-agent)
...
- Multi-tenant isolation (emergex is single-user)
...
| Role | Interest |
|------|---------|
| **emergex users** | Better context persistence across sessions; smarter agent |
...
| **emergex-as-product** | Key differentiator vs competitors (Cursor, Aider, Claude Code) |
