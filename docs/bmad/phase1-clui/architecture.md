# emergex CLUI -- Architecture Document
...
**Decision:** Tauri 2.0. The binary size and memory advantages align with emergex's lightweight philosophy. The Rust backend provides a natural home for process management, hotkey handling, and the permission server. The webview inconsistency risk is mitigated by targeting macOS first (WKWebView is excellent) and using CSS custom properties rather than system-dependent styling.
...
Each session tab spawns an independent `bun run packages/emergex/index.ts` process. Communication is via NDJSON over stdout. This provides:
...
The emergex engine outputs newline-delimited JSON events:
...
Sessions are saved to `~/.emergex/clui/sessions/{session-id}.json` using the `packages/specifications` session format (v2). On resume, CLUI replays the message history into the UI and spawns a new agent subprocess with the session context preloaded.
...
| Agent engine | packages/emergex | (monorepo) |
