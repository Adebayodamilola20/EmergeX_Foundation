...
- `bun run tauri dev` from `apps/clui/` launches a window with "Hello emergex CLUI"
...
- Hotkey is configurable in settings (stored in `~/.emergex/clui/config.json`)
...
**As a** developer,
**I want** a React 19 frontend with Tailwind CSS 4 configured and emergex design tokens mapped to CSS custom properties,
...
Spawning, communicating with, and managing emergex engine subprocesses.

### Story 2.1: Spawn emergex Subprocess per Tab

**As a** developer,
**I want** each session tab to spawn an independent `bun run packages/emergex/index.ts` process,
...
**Acceptance Criteria:**
- Sessions auto-save to `~/.emergex/clui/sessions/{id}.json` on every message
...
- Sessions older than 7 days are archived (moved to `~/.emergex/clui/archive/`)
...
- Default model configurable in `~/.emergex/clui/config.json`
...
**Acceptance Criteria:**
- Rules stored in `~/.emergex/clui/permissions.json`
...
- Changes save immediately to `~/.emergex/clui/config.json`
