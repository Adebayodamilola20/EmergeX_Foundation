# emergex CLUI -- Product Requirements Document
...
- **US-001:** As a developer, I want to press Alt+Space from any application to toggle the emergex overlay so I never leave my editor to interact with the agent.
...
| **Agent subprocess management** | Spawn `bun run packages/emergex/index.ts` per tab | US-002 |
...
| Installation method | DMG + Homebrew cask (Phase 1), `brew install emergex-clui` |
...
| Package | Integration |
|---------|------------|
| `packages/emergex` | Spawned as Bun subprocess per session tab |
