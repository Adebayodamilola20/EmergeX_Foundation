...
emergex CLUI transforms the existing emergex terminal-only experience into a native desktop overlay application. Where the current TUI lives entirely inside a terminal emulator and requires users to context-switch away from their editor, CLUI provides a floating, always-available panel toggled with a single keystroke. The agent remains local-first, running Ollama/OpenRouter/LM Studio models through the existing `packages/emergex` engine -- no cloud dependency, no usage caps.
...
2. **Multi-session concurrency.** Developers run multiple agent sessions in tabs -- one per task, repo, or experiment -- each backed by an independent emergex subprocess.
...
4. **Preserve the engine.** CLUI does not rewrite the agent. It wraps `packages/emergex` as a subprocess, consuming its NDJSON event stream. All 33 existing packages remain untouched.
...
- Share emergex configurations across a team (model presets, tool permissions, project memory).
...
- Want a visual layer on top of the emergex engine to understand what the agent is doing (ThinkingView, evidence badges, plan kanban).

---

## Key Differentiators from CLUI-CC

| Dimension | CLUI-CC (lcoutodemos) | emergex CLUI |
|-----------|----------------------|------------|
| **Agent engine** | Claude Code CLI (Anthropic SaaS) | `packages/emergex` (local LLMs, OpenRouter, LM Studio) |
...
| **Agent protocol** | `claude -p --output-format stream-json` | `bun run packages/emergex/index.ts` with NDJSON events |
...
2. **Existing 33 packages.** CLUI must not fork or modify any existing package. It consumes `packages/emergex` as a black-box subprocess.
...
6. **No Electron.** The decision is Tauri 2.0. Electron is explicitly rejected for binary size, memory usage, and philosophical alignment with emergex's lightweight ethos.
