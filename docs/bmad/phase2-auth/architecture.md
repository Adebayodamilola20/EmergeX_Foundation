...
| OAuth Provider | GitHub | 100% of emergex users are developers, GitHub is universal |
...
```
Step 1: User runs `emergex auth login`
  |
  v
Step 2: CLI calls Clerk's OAuth device authorization endpoint
        POST https://clerk.emergex.app/v1/oauth/device/authorize
...
Step 3: Clerk returns:
        {
          device_code: "xyz...",
          user_code: "ABCD-1234",
          verification_uri: "https://emergex.app/auth/device",
          verification_uri_complete: "https://emergex.app/auth/device?code=ABCD-1234",
          expires_in: 900,
          interval: 5
        }
  |
...
Step 4: CLI displays instructions and auto-opens browser:
        "Opening browser... Visit https://emergex.app/auth/device"
...
Step 6: CLI polls for completion every 5 seconds:
        POST https://clerk.emergex.app/v1/oauth/device/token
...
Step 7: On success, Clerk returns JWT. CLI stores in Keychain:
        security add-generic-password -a "emergex" -s "emergex-auth-token" -w "<jwt>"
  |
  v
...
```bash
# Store
security add-generic-password -a "emergex" -s "emergex-auth-token" -w "$JWT" -U

# Retrieve
security find-generic-password -a "emergex" -s "emergex-auth-token" -w

# Delete
security delete-generic-password -a "emergex" -s "emergex-auth-token"
```
...
### Agent Session Start (`packages/emergex/agent.ts`)
