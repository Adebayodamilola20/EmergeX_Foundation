...
- Initialize package with `package.json` following `@emergex/secrets` pattern
...
**Acceptance Criteria:**
- `import { getClerkClient } from "@emergex/auth"` works
...
**Acceptance Criteria:**
- `emergex auth login` starts device flow and displays code
...
- `emergex auth logout` clears token from both stores
...
**Acceptance Criteria:**
- All types exported from `@emergex/auth`
- No `any` types
...
**Goal:** Wire auth into the CLI entry point with `emergex auth *` commands.

### Story 3.1: `emergex auth login` Command
...
### Story 3.2: `emergex auth logout` Command
...
### Story 3.3: `emergex auth status` Command
...
- Anonymous users see "Not logged in. Run `emergex auth login`"
- Usage data fetched from Convex if online

### Story 3.4: `emergex auth whoami` Command
