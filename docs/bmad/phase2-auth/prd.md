...
Add authentication, user accounts, and cloud persistence to emergex Code using Clerk (auth provider) and Convex (real-time database). The system must preserve emergex's local-first philosophy while enabling optional cloud features.
...
> As a developer, I want to log into emergex from my terminal without leaving the CLI, so that I can authenticate quickly on headless machines or SSH sessions.

Acceptance Criteria:
- Running `emergex auth login` starts the device code flow
...
- Profile viewable via `emergex auth whoami`

**US-003: Session Token Management**
> As a returning user, I want emergex to remember my login, so that I don't have to authenticate every time I open a terminal.
...
- `emergex auth logout` clears all stored tokens

**US-004: Anonymous Mode**
> As a privacy-conscious user, I want to use emergex without any account, so that I retain full local functionality.

Acceptance Criteria:
- emergex works fully without login (current behavior preserved)
...
- Daily aggregation available via `emergex auth status`
...
> As a user with multiple machines, I want my model preferences synced, so that emergex behaves consistently everywhere.
...
```
User                    CLI (emergex)              Clerk API              Browser
 |                         |                        |                      |
 |  emergex auth login       |                        |                      |
 |------------------------>|                        |                      |
 |                         |  POST /device/code     |                      |
