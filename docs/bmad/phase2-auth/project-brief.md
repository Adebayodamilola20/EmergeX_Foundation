...
## Why Auth Matters for emergex

emergex Code is currently a single-user, local-only tool. There is no identity system — just a `.emergex/user.json` file and a machine-fingerprinted secrets vault. This blocks every growth vector:
...
- Use emergex locally with Ollama or OpenRouter.
...
- 2-20 developers sharing an emergex deployment.
...
emergex's core promise is **local-first, no lock-in**. Auth must not break this:
