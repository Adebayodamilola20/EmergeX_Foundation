...
Add local Speech-to-Text to emergex Code, completing the voice loop (TTS output already exists). Users press a key to speak, Whisper transcribes locally, and text appears in the command input.
...
- Configurable via `.emergex/config.json`
...
- Model stored in `~/.emergex/models/whisper/`
...
- emergex works perfectly fine without voice — it's opt-in
...
- "Hey emergex" wake word for hands-free activation
...
**As a** developer using emergex for the first time with voice,
...
- [ ] Voice state persists in `.emergex/config.json`
...
// .emergex/config.json
{
  "voice": {
    ...
    "modelsPath": "~/.emergex/models/whisper"
  }
}
```
