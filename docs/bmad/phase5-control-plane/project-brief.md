...
Transform emergex from a single-user CLI tool into a multi-tenant platform where every user gets their own `username.emergex.app` subdomain. The control plane provides admin observability across all tenants, usage-based billing infrastructure, and operational monitoring.

## Goals

1. **Multi-Tenant Architecture** — Each user is a tenant with isolated data, configurable plans, and subdomain routing (`username.emergex.app`).
...
- Subdomain routing resolves `username.emergex.app` to the correct tenant context
...
- Custom domain support (only `*.emergex.app`)
