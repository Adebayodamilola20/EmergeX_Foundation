...
Each extension has an `emergex-extension.json` manifest at its root:

```typescript
interface ExtensionManifest {
  name: string;               // unique identifier, e.g. "emergex-docker"
...
1. **Install** - `emergex ext install <path|npm-package>` copies/links to `~/.emergex/extensions/<name>/`
...
5. **Unload** - `emergex ext remove <name>` unhooks and deletes
...
```
emergex ext install <path|package>   # install extension
emergex ext remove <name>            # uninstall
emergex ext list                     # show installed + status
emergex ext enable <name>            # re-enable disabled
emergex ext disable <name>           # disable without removing
```
...
**Modify:**
- `packages/emergex/agent.ts` - wire extension hooks into agent event loop (~20 lines)
- `packages/emergex/tools.ts` - merge extension tools into tool set (~15 lines)
- `packages/permissions/policy-engine.ts` - per-extension sandbox creation (~20 lines)
- `bin/emergex.ts` - add `ext` subcommand (~30 lines)
