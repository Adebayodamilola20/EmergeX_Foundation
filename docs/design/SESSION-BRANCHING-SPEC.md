...
**Create:**
- `packages/emergex/session-branching.ts` - DAG construction, fork, switch, prune (~200 lines)
- `packages/emergex/branch-summarizer.ts` - LLM summarization of branches (~60 lines)

**Modify:**
- `packages/emergex/session-sync.ts` - add branchId/parentId to message serialization (~20 lines)
- `packages/emergex/agent.ts` - wire `/fork` and `/switch` commands (~30 lines)
