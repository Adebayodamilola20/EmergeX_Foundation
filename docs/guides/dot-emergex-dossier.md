# ~/.emergex — Full Dossier

> Generated 2026-03-14. Complete analysis of the `~/.emergex` directory: structure, schemas, data, and runtime behavior.
...
| Working dirs | Mix of project root + temp dirs (`/tmp/emergex-test-*`, `/var/folders/.../T/emergex-test-*`) |
...
User prompt
    ...
    ▼
┌──────────────┐     writes JSONL      ┌──────────────────────┐
│  Agent Loop  │ ──────────────────────▶│ ~/.emergex/sessions/   │
│  (packages/  │                        │   session_*.jsonl     │
│   eight/)    │                        └──────────────────────┘
│              │
│  on exit:    │     writes JSON        ┌──────────────────────┐
│  reporter    │ ──────────────────────▶│ ~/.emergex/reports/    │
│              │                        │   report_*.json       │
└──────────────┘                        └──────────────────────┘
        │
        │ reads config
        ▼
  ~/.emergex/providers.json    (which model/provider to use)
  ~/.emergex/hooks.json        (lifecycle hooks to fire)
```
