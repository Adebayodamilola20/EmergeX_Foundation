...
Use **GitHub Projects (new)** for roadmap and backlog so triage, priority, and ownership stay visible next to code. Issues stay in `8gi-foundation/emergex-code`; the Project is the planning surface.

**Live org project:** [emergex-code roadmap](https://github.com/orgs/8gi-foundation/projects/1) (project `1`). Linked to `8gi-foundation/emergex-code`. Epic **#1076** and issues **#1077–#1093** are already on the board.
...
1. **Create** (org-owned project, visible to the org):
   ...
   ```bash
   gh project create --owner 8gi-foundation --title "emergex-code roadmap"
   ```
...
2. **Link** the repository so it appears under the repo’s Projects tab:
   ...
   ```bash
   cd /path/to/emergex-code
   gh project link <PROJECT_NUMBER> --owner 8gi-foundation --repo 8gi-foundation/emergex-code
   ```
...
Defaults: owner `8gi-foundation`, repo `8gi-foundation/emergex-code`, issues `1076` and `1077–1093`. Override:

```bash
export GITHUB_PROJECT_OWNER=8gi-foundation
export GITHUB_REPO=8gi-foundation/emergex-code
