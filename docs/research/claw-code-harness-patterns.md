...
**Related:** GitHub [#1076](https://github.com/8gi-foundation/emergex-code/issues/1076). Local clone (optional): `git clone https://github.com/instructkr/claw-code.git`
...
|-------|-------------------|--------------|------------------------|
| **Workspace context** | `PortContext` (`context.py`): roots for src, tests, assets, optional archive | Single object answers “what is on disk?” for reports | `emergex doctor`, workspace index; align with `docs/REPO-CONTEXT-SPEC.md` |
| **Setup / bootstrap** | `WorkspaceSetup` + `SetupReport` (`setup.py`): Python version, platform, ordered `startup_steps()` | Deterministic startup narrative | First-run and `bun run` health; tie to `packages/emergex` init |
...
| **Command graph** | `CommandGraph` (`command_graph.py`): builtins vs plugin-like vs skill-like | **Taxonomy** of slash commands | BMAD slash commands, skills registry [#983](https://github.com/8gi-foundation/emergex-code/issues/983) |
...
| **Tool filtering** | `get_tools(simple_mode, include_mcp, permission_context)` (`tools.py`) | **Progressive disclosure**: shrink surface | Map to NemoClaw + ToolGate [#988](https://github.com/8gi-foundation/emergex-code/issues/988) |
| **Execution registry** | `ExecutionRegistry` (`execution_registry.py`): unified lookup for mirrored command/tool by name | **Single dispatch table** | `packages/emergex/tools.ts` registry pattern; one place to audit |
...
| **Bootstrap session** | `bootstrap_session()` → `RuntimeSession.as_markdown()` | One **exportable** artifact: context + setup + routing + exec + stream placeholders | Support bundle, CI artifact, incident export [#981](https://github.com/8gi-foundation/emergex-code/issues/981) |
| **Query engine** | `QueryEnginePort` (`query_engine.py`): turn caps, token budget, compaction, `TurnResult` | **Stateful loop** with stop reasons | Align with `docs/COMPACTION-SPEC.md`, loop detection [#975](https://github.com/8gi-foundation/emergex-code/issues/975) closed |
| **Transcript** | `TranscriptStore.compact(keep_last)` (`transcript.py`) | **Sliding window** without full drop | Session compaction policy |
| **Session persistence** | `StoredSession` JSON + `.port_sessions` (`session_store.py`) | Minimal durable session | Team sessions [#1051](https://github.com/8gi-foundation/emergex-code/issues/1051), daemon [#956](https://github.com/8gi-foundation/emergex-code/issues/956) |
...
| **Remote modes** | `remote_runtime.py`, `direct_modes.py` | Named transport modes (placeholders) | SSH, remote worker, deep links: compare ACP [#974](https://github.com/8gi-foundation/emergex-code/issues/974), RPC `docs/RPC-MODE-SPEC.md` |
...
1. **Everything is inspectable.** CLI subcommands that dump Markdown (`summary`, `manifest`, `setup-report`, graphs) lower support cost. Eight: `emergex capabilities`, `emergex explain-route "<text>"`.
...
Tracked as **detailed GitHub issues** (epic: [#1076](https://github.com/8gi-foundation/emergex-code/issues/1076)):

| Priority | Issue | Title |
|----------|--------|--------|
| P0 adj. | [#1083](https://github.com/8gi-foundation/emergex-code/issues/1083) | Progressive tool surface (safe subset + MCP toggle) |
| P1 | [#1077](https://github.com/8gi-foundation/emergex-code/issues/1077) | Route-debug CLI |
| P1 | [#1078](https://github.com/8gi-foundation/emergex-code/issues/1078) | Exportable support bundle (Markdown) |
| P2 | [#1079](https://github.com/8gi-foundation/emergex-code/issues/1079) | CI internal capability parity check |
| P2 | [#1080](https://github.com/8gi-foundation/emergex-code/issues/1080) | Bootstrap pipeline docs + tests |
| P3 | [#1084](https://github.com/8gi-foundation/emergex-code/issues/1084) | Labeled cost attribution events |

**Also filed:** [#1081](https://github.com/8gi-foundation/emergex-code/issues/1081) prefetch, [#1082](https://github.com/8gi-foundation/emergex-code/issues/1082) system prompt bootstrap segment, [#1085](https://github.com/8gi-foundation/emergex-code/issues/1085) unified execution registry, [#1086](https://github.com/8gi-foundation/emergex-code/issues/1086) structured agent history, [#1087](https://github.com/8gi-foundation/emergex-code/issues/1087) TUI stream events, [#1088](https://github.com/8gi-foundation/emergex-code/issues/1088) transcript sliding window, [#1089](https://github.com/8gi-foundation/emergex-code/issues/1089) central TS types, [#1090](https://github.com/8gi-foundation/emergex-code/issues/1090) command taxonomy, [#1091](https://github.com/8gi-foundation/emergex-code/issues/1091) workspace context report, [#1092](https://github.com/8gi-foundation/emergex-code/issues/1092) trust-gated deferred init, [#1093](https://github.com/8gi-foundation/emergex-code/issues/1093) extension subsystem manifest.

**Existing issues** still apply: ToolGate [#988](https://github.com/8gi-foundation/emergex-code/issues/988), skills registry [#983](https://github.com/8gi-foundation/emergex-code/issues/983), structured logging [#981](https://github.com/8gi-foundation/emergex-code/issues/981), sessions [#1051](https://github.com/8gi-foundation/emergex-code/issues/1051), daemon [#956](https://github.com/8gi-foundation/emergex-code/issues/956), sandbox [#993](https://github.com/8gi-foundation/emergex-code/issues/993).
