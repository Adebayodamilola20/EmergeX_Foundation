...
```typescript
// packages/emergex/compaction.ts
...
User config in `~/.emergex/config.json`:
...
**Files to create:**
- `packages/emergex/compaction.ts` - all compaction logic (estimator, cut point, summarizer, file tracker)

**Files to modify:**
- `packages/emergex/agent.ts` - add compaction check after each assistant response, inject summary into messageHistory
- `packages/emergex/types.ts` - add CompactionConfig to AgentConfig
