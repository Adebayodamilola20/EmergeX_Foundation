...
This is the contract between the brain (emergex Code daemon) and the interfaces (emergex.app, emergex OS, Telegram, etc.).
...
Authentication is optional. When `daemon.authToken` is set in `~/.emergex/config.json`, clients must authenticate before sending any other message.
...
On graceful shutdown (SIGTERM/SIGINT), the daemon writes active session metadata to `~/.emergex/daemon-state.json`. On restart, clients can resume sessions by ID. The Agent instance is recreated, but conversation history must be restored from the client side or from memory.

Cron jobs persist to `~/.emergex/cron.json` and survive restarts. Missed jobs are caught up on startup (if gap exceeds 2x the interval).
