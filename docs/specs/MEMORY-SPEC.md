...
Detailed implementation spec for the emergex memory layer. Incorporates patterns from FoodstackOS, Honcho (plastic-labs), and Supermemory research.
...
**Where it gets injected:** The representation becomes part of `USER_CONTEXT_SEGMENT` in `packages/emergex/prompts/system-prompt.ts`:
...
**Files:** `packages/memory/representations.ts` (new, ~70 lines), `packages/emergex/prompts/system-prompt.ts` (+10 lines)
...
| `packages/emergex/prompts/system-prompt.ts` | Modify | ~10 | Inject peer representation |
...
//     architecture. His top priorities are emergex OS launch and Nick's Jr app."
```
...
| `packages/emergex/prompts/system-prompt.ts` | Modify | ~15 | Inject auto-context + representation |
