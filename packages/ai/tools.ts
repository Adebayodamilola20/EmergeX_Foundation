/**
 * emergex AI - Tool Definitions (AI SDK format)
 *
...
const spawnAgent = tool({
  description:
    "Spawn a background agent. Use runtime='claude' for complex tasks that need a stronger model, runtime='emergex' for standard tasks, runtime='shell' for simple commands.",
  inputSchema: z.object({
    task: z.string().describe("Task description for the background agent to execute"),
    runtime: z.enum(["emergex", "claude", "shell"]).optional().describe("Runtime to use: 'emergex' (default, internal agent), 'claude' (Claude CLI for complex tasks), 'shell' (sh -c for simple commands)"),
    model: z.string().optional().describe("Model to use (only for emergex runtime)"),
...
const checkAgent = tool({
  description:
    "Check the status and result of a spawned background agent by ID. Works with all runtimes (emergex, claude, shell). Returns status and result if done.",
  inputSchema: z.object({
...
const listAgents = tool({
  description:
    "List all spawned background agents (emergex, claude, shell) with their status. Shows unified overview across all runtimes.",
  inputSchema: z.object({}),
  execute: async () => {
    try {
...
const writeNotesTool = tool({
  ...
  execute: async ({ title, content, append }) => {
    ...
    const dataDir = join(process.env.HOME || "~", ".emergex", "tabs");
...
/**
 * All emergex tools in AI SDK format.
