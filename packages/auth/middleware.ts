/**
 * @emergex/auth — Auth Middleware
 *
 * Middleware for protecting HTTP endpoints (if emergex exposes any local HTTP APIs).
...
export async function requireAuth(feature?: string): Promise<AuthUser> {
  ...
  if (state.state !== "authenticated") {
    ...
    throw new Error(
      `Authentication required${featureMsg}. Run \`emergex auth login\` to sign in.`,
    );
  }
  ...
  if (planHierarchy[user.plan] < planHierarchy[requiredPlan]) {
    ...
    throw new Error(
      `${requiredPlan} plan or higher required for${featureMsg}. ` +
        `You are on the ${user.plan} plan. Upgrade at https://emergex.app/pricing`,
    );
  }
