/**
 * emergex CI Review - AI code review. Ollama local, OpenRouter fallback.
 */
...
async function tryOpenRouter(diff: string): Promise<string | null> {
  ...
  try {
    const res = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${key}`, "HTTP-Referer": "https://emergex.dev", "X-Title": "emergex Code Review" },
