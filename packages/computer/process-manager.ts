 ...
 * - Safe list is user-configurable via .emergex/safe-apps.json
 */
...
function getSafeListPath(): string {
  const dataDir = process.env.EIGHT_DATA_DIR || path.join(os.homedir(), ".emergex");
...
function canQuit(name: string): string | null {
  ...
  if (SELF_PROCESSES.has(name)) {
    return `BLOCKED: "${name}" is part of the emergex runtime - cannot terminate self`;
  }
  return null;
}
