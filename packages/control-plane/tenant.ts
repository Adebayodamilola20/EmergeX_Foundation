/**
 * @emergex/control-plane — Tenant Management
 *
 ...
 * Extracts the subdomain from `username.emergex.app` or `username.localhost`.
 */
export function resolveSubdomain(hostname: string): string | null {
  // Production: username.emergex.app
  const prodMatch = hostname.match(/^([a-z0-9][a-z0-9-]+[a-z0-9])\.emergex\.app$/);
