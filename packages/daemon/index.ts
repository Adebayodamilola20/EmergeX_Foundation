 ...
 * Logs to ~/.emergex/daemon.log. Graceful shutdown on SIGTERM/SIGINT.
 */
...
async function main(): Promise<void> {
  ...
  if (!existingJobs.find((j: any) => j.id === "daily-ceo-summary")) {
    addJob({
      ...
      expression: "0 9 * * *",
      type: "agent-prompt",
      payload: "Generate a brief daily summary: list completed tasks, open PRs, and any pending work from the task registry at ~/.emergex/tasks.json",
      enabled: true,
      lastRun: null,
      nextRun: null,
      recurring: true,
