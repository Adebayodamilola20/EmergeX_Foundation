  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      const cosPool = new AgentPool({
        ...
        workingDirectory: process.env.HOME ? `${process.env.HOME}/.emergex/workspace` : "/app",
  ...
  private async handleTelegramMessage(text: string, chatId: number): Promise<void> {
    ...
    if (text === "/logs") {
      try {
        ...
        const logs = execSync("tail -30 /root/.emergex/daemon.log 2>/dev/null || echo 'No log file'", { encoding: "utf-8", timeout: 5000 });
