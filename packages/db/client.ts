/**
 * @emergex/db — Convex Client Wrapper for Bun
 *
 ...
 *
 * Usage:
 *   import { getConvexClient, withConvex } from "@emergex/db";
 *
...
export class ConvexClientWrapper {
  ...
  private async ensureConnected(): Promise<ConvexClient | null> {
    ...
    if (!this.config.url) {
      console.warn(
        "[emergex/db] No CONVEX_URL configured. Running in offline mode.",
      );
    ...
    try {
      ...
      console.warn(
        `[emergex/db] Failed to connect to Convex: ${error instanceof Error ? error.message : String(error)}`,
      );
      return null;
    }
  }
  ...
  async query<F extends FunctionReference<"query">>(
    ...
    try {
      ...
      console.warn(
        `[emergex/db] Query failed: ${error instanceof Error ? error.message : String(error)}`,
      );
      return null;
    }
  }
  ...
  async mutation<F extends FunctionReference<"mutation">>(
    ...
    try {
      ...
      console.warn(
        `[emergex/db] Mutation failed: ${error instanceof Error ? error.message : String(error)}`,
      );
  ...
  private async flushQueue(): Promise<void> {
    ...
    for (const { fn, args } of queue) {
      try {
        ...
        console.warn(
          `[emergex/db] Failed to flush queued mutation: ${error instanceof Error ? error.message : String(error)}`,
        );
