...
export class Agent {
  ...
  constructor(config: AgentConfig) {
    ...
    // Check if first run — if .emergex/user.json doesn't exist, flag for onboarding
    ...
    if (this.onboarding.needsOnboarding()) {
      ...
      console.log("[emergex] First run detected — onboarding available. The agent can ask setup questions.");
    }
    ...
    // Inject the emergex personality voice into the system prompt
...
Maintain a tone that is sophisticated yet approachable — like a well-dressed engineer who happens to be brilliant.\n`;
    ...
    if (vault.has("TELEGRAM_BOT_TOKEN") && !getActiveTelegramBot()) {
      ...
      if (telegramToken) {
        ...
        startTelegramBot(telegramToken, this, {
          ...
          console.log(`[emergex] Telegram auto-start failed: ${err.message}`);
        });
      }
    }

    // ── Extensions: Load from ~/.emergex/extensions/ ──────────────────
  ...
  async chat(userMessage: string, imageBase64?: string, imageMimeType?: string): Promise<string> {
    ...
    if (needsPlanningGate) {
      ...
      this.messageHistory.push({
        role: "user",
        content:
          `[PLANNING REQUIRED] Before executing ANY tools, you MUST output your plan as:\nPLAN:\n1. [step]\n2. [step]\n3. [step]\nAfter outputting your plan, YOU MUST IMMEDIATELY execute the first tool to begin step 1. Do NOT stop.`,
      });
    } else {
    ...
    try {
      ...
      if (this.infiniteModeActive && err instanceof Error) {
        ...
        if (severity !== "fatal") {
          console.log(`[emergex:heal] Attempting recovery for ${severity} error: ${errMsg.slice(0, 80)}`);
          try {
  ...
  abort(): void {
    if (this.abortController) {
      ...
      this.abortController = null;
      console.log("[emergex] Generation aborted by user");
    }
  }
  ...
  enableInfiniteMode(task: string, config?: { maxIterations?: number; maxTimeMs?: number }): InfiniteRunner {
    ...
    console.log(`[emergex] Infinite mode enabled for task: ${task}`);
  ...
  disableInfiniteMode(): void {
    ...
    console.log("[emergex] Infinite mode disabled");
  }
  ...
  /**
   * Read the syncToConvex flag from .emergex/config.json.
  ...
  restoreFromCheckpoint(messages: Array<{ role: string; content: string }>): void {
    ...
    console.log(`[emergex] Restored ${messages.length} messages from checkpoint`);
  }
