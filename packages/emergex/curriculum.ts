/**
 * emergex Code - Curriculum Skills
 *
...
const EIGHT_ARCHITECTURE: Curriculum = {
  id: "emergex-architecture",
  title: "How Eight Works Internally",
  description: "A deep dive into emergex-code's architecture, from CLI entry to tool execution.",
  targetAudience: "Developers contributing to or extending emergex-code",
  ...
  steps: [
    {
      ...
      prerequisites: [],
      content: [
        "emergex-code is a monorepo with packages under `packages/`.",
        "The core agent lives in `packages/emergex/` — it owns the agentic loop, tool dispatch, and provider abstraction.",
      ...
      exercises: [
        "Read `packages/emergex/types.ts` and identify the `LLMClient` interface.",
        "Trace the import chain from `packages/emergex/index.ts` to see what gets exported.",
...
const WRITING_BENCHMARKS: Curriculum = {
  ...
  title: "Writing New Benchmarks for emergex",
...
export const BUILTIN_CURRICULA: Record<string, Curriculum> = {
  "emergex-architecture": EIGHT_ARCHITECTURE,
