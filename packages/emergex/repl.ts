...
export async function startREPL(config?: Partial<AgentConfig>) {
  ...
  const configPath = path.join(os.homedir(), ".emergex", "config.json");
...
`);
  ...
  if (!(await agent.isReady())) {
    ...
    console.log("   Then run: npx emergex-code");
    process.exit(1);
  }
...
function printHelp(): void {
  console.log(`
\x1b[36memergex Commands:\x1b[0m
  ...
  if (trimmed.startsWith("/provider key ")) {
    ...
    if (!apiKey) {
      ...
      if (p.name === "ollama") {
        ...
        console.log(`  Stored in ~/.emergex/providers.json`);
      }
    }
    return true;
  }
...
async function handlePlanCommands(trimmed: string, agent: Agent): Promise<boolean> {
  ...
  if (trimmed === "/nightly") {
    const nightlyLog = path.join(os.homedir(), ".emergex", "nightly.log");
    const dreamsLog = path.join(os.homedir(), ".emergex", "dreams.log");

    console.log(`\n\x1b[36memergex Nightly Training Status:\x1b[0m\n`);
    ...
    console.log(`  Training: 2:00 AM PST daily (lockfile: /tmp/emergex-nightly.lock)`);
...
function handleMCPCommands(trimmed: string): boolean {
  if (trimmed === "/mcp") {
    ...
    if (servers.length === 0) {
      console.log("\x1b[33mNo MCP servers running.\x1b[0m Configure in ~/.emergex/mcp.json");
    } else {
...
function handlePersonalCommands(trimmed: string): boolean {
  if (trimmed === "/personal" || trimmed === "/personal status") {
    const loraDir = path.join(os.homedir(), ".emergex", "personal-lora");
...
async function handleLanguageCommands(trimmed: string): Promise<boolean> {
  ...
  if (trimmed.startsWith("/language ")) {
    ...
    if (lm.setLanguage(code)) {
      ...
      console.log(`  emergex will now respond in ${lang.name}.`);
    } else {
...
function handleHooksCommands(trimmed: string): boolean {
  if (trimmed === "/hooks") {
    ...
    if (hooks.length === 0) {
      console.log("  No hooks registered. Add hooks to ~/.emergex/hooks.json");
    }
...
async function handleSkillsCommands(trimmed: string): Promise<boolean> {
  if (trimmed === "/skills") {
    try {
      ...
      if (skills.length === 0) {
        console.log("  No skills found. Add .md files in ~/.emergex/skills/ or run from a repo with .claude/skills/*/SKILL.md");
      }
    } catch (err) {
