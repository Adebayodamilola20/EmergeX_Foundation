...
export class ToolExecutor {
  ...
  getToolDefinitions(): object[] {
    return [
      ...
      // Multi-agent orchestration
      {
        ...
        function: {
          name: "spawn_agent",
          description: "[SHELL] Launches a background agent and returns an agentId for tracking. Use runtime='claude' for complex multi-step tasks needing a stronger model, runtime='emergex' for standard coding tasks, runtime='shell' for simple one-off commands. The agent runs asynchronously - use check_agent with the returned ID to poll for results. For emergex runtime, pass model='auto:free' to auto-select the best free model.",
          parameters: {
            type: "object",
            properties: {
              task: { type: "string", description: "Task description for the background agent to execute" },
              runtime: { type: "string", enum: ["emergex", "claude", "shell"], description: "Runtime: 'emergex' (default), 'claude' (Claude CLI), 'shell' (sh -c)" },
              model: { type: "string", description: "Model to use (only for emergex runtime). Use 'auto:free' to automatically pick the best free model from OpenRouter." },
      ...
      // Memory tools
      {
        ...
        function: {
          name: "remember",
          description: "[MEMORY] Persists a fact to the specified memory layer and returns confirmation. Use 'session' for temporary context that disappears when the session ends. Use 'project' for facts about this codebase (persisted in .emergex/ - e.g., architecture decisions, user preferences for this repo). Use 'global' for cross-project knowledge (persisted in ~/.emergex/ - e.g., user's coding style, tool preferences). Keep facts concise and searchable. Pair with recall to retrieve later.",
          parameters: {
            type: "object",
            properties: {
    ...
    switch (toolName) {
      ...
      case "spawn_agent":
        return this.handleSpawnAgent(
          args.task as string,
          (args.runtime as "emergex" | "claude" | "shell" | undefined),
  ...
  private async handleSpawnAgent(
    task: string,
    runtime?: "emergex" | "claude" | "shell",
    model?: string,
    timeout?: number
  ): Promise<string> {
    try {
      const effectiveRuntime = runtime || "emergex";
  ...
  private async handleCheckAgent(agentId: string): Promise<string> {
    try {
      ...
      // Default: check emergex agent pool
      const { getAgentPool } = await import("../orchestration");
      const pool = getAgentPool();
  ...
  private async handleMCPListTools(): Promise<string> {
    try {
      ...
      if (tools.length === 0) {
        return "No MCP tools available. Configure servers in ~/.emergex/mcp.json";
      }
