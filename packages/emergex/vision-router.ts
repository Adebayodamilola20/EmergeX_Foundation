 ...
 *
 * Pipeline:
 * 1. If user has configured a preferred model in .emergex/config.json → use it
...
/**
 * User-configurable vision settings from .emergex/config.json
 */
...
/**
 * Load vision config from .emergex/config.json.
...
export function loadVisionConfig(projectDir?: string): VisionConfig {
  const candidates = [
    projectDir ? path.join(projectDir, ".emergex", "config.json") : null,
    path.join(process.cwd(), ".emergex", "config.json"),
    path.join(process.env.HOME || "", ".emergex", "config.json"),
...
/**
 * Save vision config back to .emergex/config.json
 */
export function saveVisionConfig(config: Partial<VisionConfig>, projectDir?: string): boolean {
  const configPath = path.join(projectDir || process.cwd(), ".emergex", "config.json");
  try {
 ...
 *
 * Priority:
 * 0. User-configured model in .emergex/config.json (if set and available)
