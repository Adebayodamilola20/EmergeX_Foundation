...
export async function doctor(_args: string[]): Promise<void> {
  ...
  try {
    ...
    checks.push({ name: "Agent module", pass: true, detail: "packages/emergex/agent.ts loaded" });
  } catch (err) {
    checks.push({
      name: "Agent module",
      pass: false,
  ...
  if (allPass) {
    ...
    console.log("  \x1b[33mSome checks failed — emergex may not work fully.\x1b[0m\n");
  }
}
