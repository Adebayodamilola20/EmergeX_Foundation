 ...
 *
 * Runs a headless emergex session and captures the full result.
...
function createTempWorkdir(label: string): string {
  ...
  const dir = path.join(os.tmpdir(), `emergex-test-${slug}-${Date.now()}`);
...
export async function run(args: string[]): Promise<void> {
  ...
  if (opts.workdir) {
    ...
    if (isInsideGitRepo(workdir) && !workdir.startsWith(os.tmpdir()) && !workdir.startsWith("/tmp")) {
      ...
      console.error(`         emergex writes files — this would overwrite your project.`);
