...
export const loggingHook: Omit<Hook, "id"> = {
  type: "afterTool",
  name: "Tool Logger",
  description: "Logs all tool executions to ~/.emergex/logs/tools.log",
  mode: "shell",
  command: 'echo "[$(date -Iseconds)] Tool: {tool}" >> ~/.emergex/logs/tools.log',
  enabled: false,
...
export const commandLoggingHook: Omit<Hook, "id"> = {
  type: "afterCommand",
  name: "Command Logger",
  description: "Logs all shell commands to ~/.emergex/logs/commands.log",
  mode: "shell",
  command: 'echo "[$(date -Iseconds)] Command: {command} (exit: {exitCode})" >> ~/.emergex/logs/commands.log',
  enabled: false,
  continueOnError: true,
  async: true,
};
...
export const errorLoggingHook: Omit<Hook, "id"> = {
  type: "onError",
  name: "Error Logger",
  description: "Logs all errors to ~/.emergex/logs/errors.log",
  mode: "shell",
  command: 'echo "[$(date -Iseconds)] Error: {error}" >> ~/.emergex/logs/errors.log',
  enabled: false,
  continueOnError: true,
};
...
export const macosNotificationHook: Omit<Hook, "id"> = {
  ...
  mode: "shell",
  command: 'osascript -e \'display notification "Task completed" with title "emergex Code"\'',
  enabled: false,
  continueOnError: true,
  async: true,
};
...
export const telegramNotificationHook: Omit<Hook, "id"> = {
  ...
  mode: "shell",
  command: 'curl -s -X POST "https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/sendMessage" -d "chat_id=$TELEGRAM_CHAT_ID&text=emergex: Task completed"',
  enabled: false,
  continueOnError: true,
  async: true,
};
...
export const backupBeforeEditHook: Omit<Hook, "id"> = {
  ...
  functionBody: `
    if (context.tool === 'edit_file' || context.tool === 'write_file') {
      ...
      if (path) {
        const fs = require('fs');
        const backupPath = path + '.emergex-backup';
