...
const YAML_HOOKS_PATH = path.join(
  process.env.EIGHT_DATA_DIR || path.join(os.homedir(), ".emergex"),
  "hooks.yaml"
);
...
export class HookManager {
  ...
  constructor(configPath?: string) {
    this.configPath = configPath || path.join(os.homedir(), ".emergex", "hooks.json");
  ...
  /**
   * Load YAML hook configs from ~/.emergex/hooks.yaml
   */
  ...
  private async executeScriptHook(hook: Hook, context: HookContext): Promise<string> {
    ...
    const scriptPath = path.isAbsolute(hook.scriptPath)
      ? hook.scriptPath
      : path.join(os.homedir(), ".emergex", "hooks", hook.scriptPath);
