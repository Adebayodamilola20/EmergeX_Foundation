...
export class MemoryManager {
  ...
  constructor(workingDirectory: string) {
    ...
    const globalDataDir = process.env.EIGHT_DATA_DIR || path.join(os.homedir(), ".emergex");
    this.projectJsonlPath = path.join(workingDirectory, ".emergex", "memory", "project.jsonl");
  ...
  async init(): Promise<void> {
    ...
    try {
      ...
      const projectDbDir = path.join(this.workingDirectory, ".emergex", "memory");
      ...
      const globalBase = process.env.EIGHT_DATA_DIR || path.join(os.homedir(), ".emergex");
