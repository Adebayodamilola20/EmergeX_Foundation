 ...
 * Rebuilt for emergex's architecture using mpv + yt-dlp + ffmpeg.
 *
 * Capabilities:
...
const HOME = homedir();
const TMP = tmpdir();
const IPC_PATH = join(TMP, "mpv-emergex-dj.sock");
const MUSIC_DIR = join(HOME, "Music", "emergex");
const RESUME_PATH = join(HOME, ".emergex", "dj-resume.json");
...
function saveResume(title: string, url: string, positionSec: number): void {
  try {
    mkdirSync(join(HOME, ".emergex"), { recursive: true });
