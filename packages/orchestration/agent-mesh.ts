 ...
 * - emergex Code instances (native)
 * - OpenCode, Cursor, Codex, etc. (via lightweight adapter)
 * - Lil EmergeX dock pet (observer + orchestrator)
 *
 * Uses filesystem-based IPC (no daemon required):
 * - ~/.emergex/mesh/registry.json - all active agents
 * - ~/.emergex/mesh/messages/<agentId>/ - per-agent inbox
 * - ~/.emergex/mesh/broadcast/ - messages to all agents
 *
...
export interface MeshAgent {
  id: string
  type: "claude-code" | "eight" | "opencode" | "cursor" | "codex" | "lil-emergex" | "custom"
  name: string
  pid: number
  cwd: string
  startedAt: number
...
const MESH_DIR = join(homedir(), ".emergex", "mesh")
