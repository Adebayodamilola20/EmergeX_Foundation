 ...
 * - "emergex": spawns an internal emergex Agent instance (default)
...
export type AgentRuntime = "emergex" | "claude" | "shell";

export interface CLIAgentOptions {
  workingDirectory?: string;
  timeout?: number; // ms, default 5 minutes
  model?: string; // only used for emergex runtime
...
export interface RunningCLIAgent {
  ...
  runtime: AgentRuntime;
  task: string;
  process: ChildProcess | null; // null for emergex (uses Agent class)
  startedAt: Date;
  completedAt?: Date;
 ...
 * - "emergex": delegates to AgentPool (returns null — handled separately)
 */
export function spawnCLIAgent(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), `emergex-cli-${runtime}-`));
