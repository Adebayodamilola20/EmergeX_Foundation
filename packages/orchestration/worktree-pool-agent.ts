 ...
 *
 * Spawns: bun run packages/emergex/index.ts --worktree <path> <prompt>
 * Result written to: <worktree>/.emergex/result.json
 */
...
export class WorktreePoolAgent {
  ...
  start(onDone: (result: WorktreeResult) => void): void {
    ...
    const logDir = join(this.task.worktreePath, ".emergex");
    ...
    this.proc = spawn(
      "bun",
      ["run", "packages/emergex/index.ts", this.task.prompt],
      {
  ...
  private finish(status: WorktreeTaskStatus, exitCode: number | null, error?: string): void {
    ...
    const resultPath = join(this.task.worktreePath, ".emergex", "result.json");
    try {
  ...
  static readResult(worktreePath: string): WorktreeResult | null {
    const p = join(worktreePath, ".emergex", "result.json");
