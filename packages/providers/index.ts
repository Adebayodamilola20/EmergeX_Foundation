/**
 * emergex Code - LLM Provider Abstraction Layer
 *
...
const PROVIDER_DEFAULTS: Record<ProviderName, ProviderConfig> = {
  "emergex": {
    name: "emergex",
    displayName: "emergex (The Infinite Gentleman)",
...
export class ProviderManager {
  ...
  constructor(settingsPath?: string) {
    this.settingsPath = settingsPath || path.join(os.homedir(), ".emergex", "providers.json");
  ...
  private loadSettings(): ProviderSettings {
    try {
      if (fs.existsSync(this.settingsPath)) {
        ...
        return {
          activeProvider: saved.activeProvider || "emergex",
  ...
  private async chatOpenAICompatible(
    ...
    if (provider.name === "openrouter") {
      headers["HTTP-Referer"] = "https://emergex.app";
      headers["X-Title"] = "emergex Code";
    }
