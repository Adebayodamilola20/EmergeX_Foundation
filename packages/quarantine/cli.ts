#!/usr/bin/env bun
/**
 * emergex Code - Quarantine CLI
 *
 * Command-line interface for managing skill quarantine.
 *
 * Usage:
 *   emergex quarantine <command> [args]
 *
 * Commands:
...
const HELP = `
🔒 emergex Quarantine - Skill Security System

Usage: emergex quarantine <command> [args]
...
Examples:
  emergex quarantine add https://github.com/user/skill
  emergex quarantine scan skill-1234567890-abc123
  emergex quarantine list pending
  emergex quarantine release skill-1234567890-abc123
  emergex quarantine reject skill-1234567890-abc123 "Suspicious network calls"
`;
  ...
  try {
    switch (command) {
      case "add": {
        ...
        console.log(`\nNext: emergex quarantine scan ${entry.id}`);
        break;
      }
      ...
      case "scan": {
        ...
        if (entry?.status === "rejected") {
          ...
          console.log(`\nNext: emergex quarantine release ${id}`);
        } else if (result.verdict === "REVIEW_REQUIRED") {
          console.log(`\nNext: Review findings, then:`);
          console.log(`  emergex quarantine release ${id}  (to approve)`);
          console.log(`  emergex quarantine reject ${id}   (to reject)`);
        }
        break;
      }
      ...
      case "release": {
        ...
        if (entry.status === "scanned") {
          console.log("\n🔧 Abstracting skill to emergex format...");
        ...
        console.log(`\n✅ Skill released! Now available in emergex.`);
        break;
      }
