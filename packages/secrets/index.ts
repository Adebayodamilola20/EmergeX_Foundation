...
function deriveKey(): Buffer {
  ...
  const salt = "emergex-vault-v1"; // static salt — key is machine-bound
...
export class SecretVault {
  ...
  constructor(vaultPath?: string) {
    this.vaultPath =
      vaultPath ?? path.join(os.homedir(), ".emergex", "vault.enc");
    this.key = deriveKey();
    this.data = this.load();
  }
