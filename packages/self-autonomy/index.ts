/**
 * emergex Code - Self-Autonomy Engine
 *
...
export class AutoGit {
  ...
  private log(message: string): void {
    if (this.verbose) {
      console.log(`[emergex:git] ${message}`);
    }
  }
  ...
  createTaskBranch(taskSlug: string, type: "feat" | "fix" | "refactor" | "self" = "feat"): string {
    ...
    if (!state.isClean) {
      this.exec(`git stash push -m "emergex-pre-branch-${timestamp}"`);
  ...
  createSnapshot(label: string): string {
    const timestamp = Date.now();
    const snapshotName = `emergex-snapshot-${label}-${timestamp}`;
...
export class SelfHeal {
  ...
  constructor(workingDirectory: string, verbose: boolean = false) {
    this.cwd = workingDirectory;
    this.verbose = verbose;
    this.memoryPath = path.join(workingDirectory, ".emergex", "context", "healing.json");
    this.memory = this.loadMemory();
  }

  private log(message: string): void {
    if (this.verbose) {
      console.log(`[emergex:heal] ${message}`);
    }
  }
...
export class SessionMemory {
  ...
  constructor(workingDirectory: string, verbose: boolean = false) {
    this.cwd = workingDirectory;
    this.contextPath = path.join(workingDirectory, ".emergex", "context");
    this.verbose = verbose;
  ...
  private log(message: string): void {
    if (this.verbose) {
      console.log(`[emergex:memory] ${message}`);
    }
  }
  ...
  logEvolution(entry: EvolutionEntry): void {
    const logFile = path.join(this.cwd, ".emergex", "evolution.log");
  ...
  async handleError(
    ...
    if (severity === "recoverable") {
      ...
      for (const alt of alternatives) {
        ...
        // For now, just log
        console.log(`[emergex:heal] Would try alternative: ${alt}`);
      }
    }
