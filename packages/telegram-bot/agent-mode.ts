/**
 * @emergex/telegram-bot — Agent Mode
 *
...
const HOME = homedir();
const LOOP_STATE = join(HOME, "emergex-code/benchmarks/autoresearch/loop-state.json");
const LEGACY_STATE = join(HOME, "iris-observatory/benchmarks/autoresearch/loop-state.json");
const CONFIG_FILE = join(HOME, ".emergex/config.json");
const RUN_PID_FILE = join(HOME, ".emergex/run.pid");
const KNOWLEDGE_DIR = join(HOME, ".emergex/intelligence");
...
const INTENT_MAP: IntentPattern[] = [
  ...
  {
    patterns: [
      /\b(status|state|health|how('?s| is)\s+(it|the bot|emergex|everything))\b/i,
  ...
  {
    patterns: [
      /\b(scores?|results?|numbers?|how did)\b.*\b(benchmark|test|round|we|it|emergex)?\b/i,
  ...
  },
  {
    patterns: [
      /\bcompare\b/i,
      /\bemergex\s+(vs|versus|against)\s+claude\b/i,
      /\bclaude\s+(vs|versus|against)\s+emergex\b/i,
      /\bhow\s+(do|does)\s+(we|emergex)\s+(compare|stack\s*up|measure)\b/i,
    ],
    action: "compare",
  },
  {
  ...
  },
  {
    patterns: [
      /\bmutation/i,
      /\blearning/i,
      /\bwhat\s+(has|have)\s+(it|we|emergex)\s+learned\b/i,
      /\baccumulated\b/i,
    ],
...
export class TelegramAgentMode {
  ...
  private async actionRunBenchmark(): Promise<ActionResult> {
    ...
    const scriptPath = join(HOME, "emergex-code/benchmarks/autoresearch/harness.ts");
  ...
  private async actionCompare(): Promise<ActionResult> {
    ...
    const baselinePath = join(HOME, ".emergex/claude-baseline.json");
  ...
  private async actionStartCompetition(): Promise<ActionResult> {
    ...
    const scriptPath = join(HOME, "emergex-code/scripts/nightly-train.ts");
    if (!existsSync(scriptPath)) {
      return {
        success: false,
    ...
    try {
      ...
      const pidDir = join(HOME, ".emergex");
