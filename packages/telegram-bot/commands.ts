/**
 * @emergex/telegram-bot — Command Handlers
 *
 * Slash commands for the emergex Telegram bot.
...
const NIGHTLY_LOG = join(HOME, ".emergex/nightly.log");
...
async function handleHelp(args: string, chatId: number, bot: any): Promise<void> {
  const lines = [
    "🤖 *EMERGEX BOT — Commands*",
    "────────────────────────────",
    "",
    "📊 *Monitoring*",
    "  `/status`  — System status dashboard",
    "  `/scores`  — Latest benchmark scores",
    "  `/compare` — emergex vs Claude comparison",
    ...
    "",
    "_Built for emergex.app — the autonomous coding agent._",
  ];
...
export const commands: CommandDefinition[] = [
  ...
  { name: "compare", description: "Show emergex vs Claude comparison", handler: handleCompare },
