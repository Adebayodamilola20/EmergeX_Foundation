/**
 * @emergex/telegram-bot — Rich Message Formatters
 *
...
export function formatBenchmarkReport(report: BenchmarkReport): string {
  ...
  // Header
  sections.push(`📊 *EMERGEX BENCHMARK REPORT*`);
...
export function formatMorningBrief(summary: OvernightSummary): string {
  ...
  sections.push(`🤖 *emergex Overnight Report*`);
  sections.push(`${horizontalRule()}`);
  sections.push("");
...
export function formatSystemStatus(status: SystemStatus): string {
  ...
  const lines: string[] = [];
  lines.push(`${statusIcon} *EMERGEX STATUS*`);
  lines.push(`${horizontalRule()}`);
  lines.push("");
  lines.push("```");
...
/**
 * Format a comparison table (emergex vs Claude).
 */
export function formatComparison(
  ...
  const lines: string[] = [];
  lines.push("⚔️ *EMERGEX vs CLAUDE*");
  lines.push(`${horizontalRule()}`);
  lines.push("");
  lines.push("```");
  lines.push(`${padRight("Benchmark", 16)} emergex Claude  Δ`);
  ...
  lines.push("");
  if (avgDelta > 5) {
    lines.push("🏆 *emergex leads by a significant margin.*");
  } else if (avgDelta > 0) {
    lines.push("📈 *emergex has a slight edge.*");
  } else if (avgDelta > -5) {
