/**
 * @emergex/telegram-bot — Main Bot Module
 *
 * Full autonomous agent bot for emergex-code.
 ...
 *
 * Usage:
 *   import { TelegramBot } from "@emergex/telegram-bot";
 *   const bot = new TelegramBot();
 *   await bot.sendMessage("Hello from emergex!");
...
export class TelegramBot {
  ...
  async startPolling(): Promise<void> {
    if (this.polling) {
      console.warn("[emergex-bot] Already polling.");
      return;
    }

    // Verify the token works
    const me = await apiCall(this.token, "getMe");
    console.log(`[emergex-bot] Connected as @${me.username} (${me.id})`);
    ...
    console.log("[emergex-bot] Polling started. Agent mode active. Listening for commands and messages...");
      ...
      try {
        ...
        for (const update of updates) {
          ...
          this.handleUpdate(update).catch((err) => {
            console.error(`[emergex-bot] Handler error: ${err.message}`);
          });
        }
      } catch (err: any) {
        if (!this.polling) break;
        console.error(`[emergex-bot] Poll error: ${err.message}`);
        // Back off on error
        await new Promise((r) => setTimeout(r, 5000));
      }
    }

    console.log("[emergex-bot] Polling stopped.");
  }
  ...
  private async handleUpdate(update: TelegramUpdate): Promise<void> {
    ...
    if (String(chatId) !== this.chatId) {
      console.log(`[emergex-bot] Ignoring message from chat ${chatId}`);
      return;
    }
