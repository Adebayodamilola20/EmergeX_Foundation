 ...
 *
 * This is the SINGLE tool that makes emergex model-agnostic.
...
// ── emergex: Meta commands ────────────────────────────

registerBuiltin({
  name: "emergex",
  summary: "emergex agent control. Subcommands: status, model, skills, outline, history.",
  usage: "emergex <subcommand> [args]",
  handler: (args, _stdin, ctx) => {
    const sub = args[0];
    if (!sub) {
      return {
        stdout: [
          "emergex — Agent Control",
          "",
          "  emergex status              — Agent status (model, history, uptime)",
          "  emergex model               — Current model",
          "  emergex model <name>        — Switch model",
          "  emergex skills              — List available skills",
          "  emergex skills <name>       — Show skill details",
          "  emergex outline <file>      — AST outline of a file",
          "  emergex history             — Conversation length",
          "  emergex help                — This help",
        ].join("\n"),
        stderr: "",
        exitCode: 0,
      };
    ...
    switch (sub) {
      case "help":
        return { stdout: builtins.get("emergex")!.handler([], "", ctx).stdout, stderr: "", exitCode: 0 };
      case "status":
      ...
      case "skills":
        try {
          const indexPath = path.join(os.homedir(), ".emergex", "skills", ".index.json");
          ...
          return { stdout: "No skills loaded. Run: emergex skills load", stderr: "", exitCode: 1 };
        } catch (err: any) {
          return { stdout: "", stderr: `skills: ${err.message}`, exitCode: 1 };
        }
      case "outline":
        if (!args[1]) return { stdout: "", stderr: "outline: usage: emergex outline <file>", exitCode: 1 };
      ...
      default:
        return { stdout: "", stderr: `emergex: unknown subcommand '${sub}'. Use: emergex help`, exitCode: 1 };
    }
  },
});
...
const OVERFLOW_DIR = path.join(os.tmpdir(), "emergex-run-output");
...
registerTool({
  ...
  inputSchema: {
    ...
    properties: {
      command: {
        type: "string",
        description: "Command to execute. Supports pipes (|), chains (&&, ||, ;). Examples: 'cat file.ts | grep TODO | wc -l', 'ls -l && echo done', 'emergex status'",
      },
    },
    required: ["command"],
  },
