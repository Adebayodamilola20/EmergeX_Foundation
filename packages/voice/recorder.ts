...
export class MicRecorder extends EventEmitter<RecorderEvents> {
  ...
  constructor(opts: RecorderOptions = {}) {
    ...
    this.options = {
      ...
      outputPath: opts.outputPath ?? join(tmpdir(), `emergex-voice-${Date.now()}.wav`),
    };
  ...
  async start(): Promise<string> {
    ...
    this.outputPath = this.options.outputPath.includes("emergex-voice-")
      ? join(tmpdir(), `emergex-voice-${Date.now()}.wav`)
