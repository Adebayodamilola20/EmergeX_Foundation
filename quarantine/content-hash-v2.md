...
3. Add to `packages/emergex/tools.ts` if the agent should be able to detect which files changed between runs.
4. Add a test in `benchmarks/categories/abilities/` to verify manifest diffing correctness.
5. Consider persisting manifests to `.emergex/cache/manifests/` for cross-session incremental builds.
...
- Default ignore list covers `.git`, `node_modules`, `.emergex`, `dist`, `.cache`, `*.log`.
