...
Consistent hash ring for distributing load across named nodes. Uses virtual nodes (vnodes) to minimise rebalancing when cluster membership changes. Supports wemergexed routing so heavier nodes absorb proportionally more traffic.
...
// Add nodes - wemergex defaults to 1
ring.addNode("node-a");
...
console.log(ring.stats()); // [{ id, wemergex, vnodes }, ...]
...
4. Add to agent tool registry in `packages/emergex/tools.ts`
