...
A lightwemergex lifecycle hook system. Consumers register named hooks in `before` or `after` phases. When an event fires, all hooks for that phase run in priority order, each receiving (and optionally mutating) a shared context object.
