...
// Chained filters
template("{title|trim|upper}", { title: "  emergex agent  " });
// => "EMERGEX AGENT"
...
- [ ] Wire into `packages/emergex/prompts/system-prompt.ts` for USER_CONTEXT_SEGMENT rendering
- [ ] Expose via agent tool so EmergeX can call it at runtime
...
The emergex codebase already does basic string interpolation inline at the call site (system-prompt.ts, memory injection, etc.). This package consolidates and upgrades that pattern. Before merging, confirm there is no duplication and identify all call sites that should migrate.
