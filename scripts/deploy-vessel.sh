#!/bin/bash
#
# Deploy Your Own emergex Vessel
...
echo "=== Deploying emergex Vessel: $APP_NAME ==="
echo "Region: $REGION"
echo ""
...
if [ -f "$HOME/emergex-vessel/Dockerfile" ]; then
  cp "$HOME/emergex-vessel/Dockerfile" "$TMPDIR/Dockerfile"
  cp "$HOME/emergex-vessel/entrypoint.sh" "$TMPDIR/entrypoint.sh"
