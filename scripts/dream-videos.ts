#!/usr/bin/env bun
/**
 * emergex Dream Sequence Generator
 *
...
function getTodaysLearnings(): string[] {
  ...
  // 1. Pull today's git commits from emergex-code
  try {
    const { execSync } = require("child_process");
    const since = new Date();
    since.setHours(0, 0, 0, 0);
    const gitLog = execSync(
      `git -C ${os.homedir()}/emergex-code log --since="${since.toISOString()}" --oneline --no-merges 2>/dev/null`,
  ...
  try {
    const learningsLog = path.join(os.homedir(), ".emergex", "benchmark-learnings.log");
...
async function main() {
  log("🌙 emergex Dream Sequence Generator — Starting");

  await sendTelegramText("🌙 <b>emergex is dreaming...</b>\n\nGenerating tonight's dream sequences. Stand by for some very professional AI introspection.");
