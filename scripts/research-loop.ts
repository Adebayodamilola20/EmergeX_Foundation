#!/usr/bin/env bun
/**
 * emergex Code - Autoresearch Competition Loop
 *
 * Runs experiments comparing emergex Code vs Claude Code,
...
async function runExperimentemergex(experiment: typeof EXPERIMENTS[0]): Promise<ExperimentResult> {
  const startTime = Date.now();

  // TODO: Actually run through emergex toolshed
  // For now, simulate with AST-first approach

  log(`[emergex] Running experiment: ${experiment.name}`);
...
async function runCompetitionRound(): Promise<void> {
  log("═══════════════════════════════════════════════════════════════");
  log("  emergex Code vs Claude Code - Competition Round");
  log("═══════════════════════════════════════════════════════════════");

  let totalTokensemergex = 0;
  let totalTokensClaude = 0;
  let totalSavedemergex = 0;
  ...
  for (const experiment of EXPERIMENTS) {
    ...
    // Run both systems
    const resultemergex = await runExperimentemergex(experiment);
    const resultClaude = await runExperimentClaude(experiment);

    // Record results
    recordResult(resultemergex);
    recordResult(resultClaude);

    // Accumulate totals
    totalTokensemergex += resultemergex.tokensUsed;
    totalTokensClaude += resultClaude.tokensUsed;
    totalSavedemergex += resultemergex.tokensSaved;

    // Log comparison
    const savings = ((resultClaude.tokensUsed - resultemergex.tokensUsed) / resultClaude.tokensUsed * 100).toFixed(1);
    log(`emergex: ${resultemergex.tokensUsed} tokens | Claude: ${resultClaude.tokensUsed} tokens | Savings: ${savings}%`);
  }
  ...
  log("  ROUND SUMMARY");
  log("═══════════════════════════════════════════════════════════════");
  log(`Total tokens - emergex: ${totalTokensemergex} | Claude: ${totalTokensClaude}`);
  log(`Total saved by emergex: ${totalSavedemergex}`);
  const overallSavings = ((totalTokensClaude - totalTokensemergex) / totalTokensClaude * 100).toFixed(1);
