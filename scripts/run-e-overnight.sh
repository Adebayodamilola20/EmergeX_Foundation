...
REPO="/Users/jamesspalding/emergex-code"
BRANCH="research/run-e-gemma4-lmstudio"
ISSUE=1232
LOG="$HOME/.emergex/nightly.log"
...
rm -f "$HOME/.emergex/run-checkpoint.json"
...
cp "$HOME/.emergex/nightly.log" "$REPO/research/run-e-nightly.log" 2>/dev/null || true
cp "$HOME/.emergex/benchmark-learnings.log" "$REPO/research/run-e-learnings.log" 2>/dev/null || true
mkdir -p "$REPO/research"

# Write a brief results summary
PASS_COUNT=$(grep -c "→ PASS" "$HOME/.emergex/nightly.log" 2>/dev/null || echo 0)
FAIL_COUNT=$(grep -c "→ FAIL" "$HOME/.emergex/nightly.log" 2>/dev/null || echo 0)
AVG=$(grep "Benchmark summary" "$HOME/.emergex/nightly.log" 2>/dev/null | tail -1 || echo "unknown")
